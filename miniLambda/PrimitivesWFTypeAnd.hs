...
{-@ lem_wf_intype_and :: () -> { pf:_ | noDefnsInRefns Empty (inType And) && isWellFormed Empty (inType And) } @-}
...
{-@ lem_wf_ty_and :: () -> { pf:_ | noDefnsInRefns Empty (ty And) && isWellFormed Empty (ty And) } @-}
lem_wf_ty_and :: () -> Proof
