...
lem_denote_sound_typ g e t p_e_t@(TBC _g b) p_g_wf th den_g_th  
  = Hex { in_cs = th, in_exp = e, 
            in_typ = t,
            val = value, -- `withProof` lem_csubst_bc th b, 
            evals = ev_the_v', -- `withProof` lem_csubst_bc th b, 
            den = den_bl_b }
        where
...
lem_denote_sound_typ g e t p_e_t@(TIC _g n) p_g_wf th den_g_th 
  = Hex { in_cs = th, in_exp = e, in_typ = t,
          val = value, evals = ev_the_v', den = den_int_n }
      where
        {-@ value :: { z:Value | Set_emp (fv z) && z = Ic n } @-}
        value     = e
...
lem_denote_sound_typ g e t p_e_t@(TPrm _g c) p_g_wf th den_g_th 
  = Hex { in_cs = th, in_exp = e, in_typ = t, 
          val = value, evals = ev_the_v', den = den_tyc_c }
  where
