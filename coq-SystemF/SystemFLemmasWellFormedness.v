...
Lemma lem_weaken_wfft' : forall (g'g : fenv) (t : ftype) (k : kind),
    WFFT g'g t k -> ( forall (g g':fenv) (x:vname) (t_x:ftype),
        g'g = concatF g g' -> uniqueF g -> uniqueF g'
...
Proof. apply ( WFFT_ind 
  ...
  - (* WFFTPoly  *) intro env; unfold in_envF; intros; 
    ...
    intros; apply not_elem_names_add_elim in H7; destruct H7;
    apply not_elem_union_elim in H8; destruct H8;
    apply (H0 a H8 g (FConsT a k g') x t_x); subst env; simpl;
    apply not_elem_concatF_elim in H9; destruct H9; intuition. 
    * apply intersect_names_add_intro_r; assumption.
    * apply set_add_elim in H10; intuition.
  - (* WFFTKind *) intros; apply WFFTKind; apply H0; assumption.
  Qed.

Lemma lem_weaken_wfft : forall (g g':fenv) (t:ftype) (k:kind) (x:vname) (t_x:ftype),
    WFFT (concatF g g') t k -> uniqueF g -> uniqueF g'
                            -> intersect (bindsF g) (bindsF g') = empty
                            -> ~ (in_envF x g) -> ~ (in_envF x g') 
                            -> WFFT (concatF (FCons x t_x g) g') t k.
...
Proof. apply ( WFFT_ind 
  ...
  - (* WFFTPoly  *) intros env k0; unfold in_envF; intros; 
    ...
    intros; apply not_elem_names_add_elim in H7; destruct H7;
    apply not_elem_union_elim in H8; destruct H8;
    apply (H0 a0 H8 g (FConsT a0 k0 g') a k); subst env; simpl;
    apply not_elem_concatF_elim in H9; destruct H9; intuition. 
    * apply intersect_names_add_intro_r; assumption.
    * apply set_add_elim in H10; intuition.
  - (* WFFTKind  *) intros; apply WFFTKind; apply H0; assumption. 
...
Lemma lem_weaken_tv_wfft : forall (g g':fenv) (t:ftype) (k_t:kind) (a:vname) (k:kind),
    WFFT (concatF g g') t k_t -> uniqueF g -> uniqueF g'
                              -> intersect (bindsF g) (bindsF g') = empty
                              -> ~ (in_envF a g) -> ~ (in_envF a g') 
                              -> WFFT (concatF (FConsT a k g) g') t k_t.
...
Lemma bound_inF_wfft : forall (x:vname) (b:ftype) (g:fenv),
    bound_inF x b g -> WFFE g -> WFFT g b Star.
Proof. intros; induction g; simpl in H; try contradiction.
  - destruct H; inversion H0; subst x0; subst t;
    assert (FCons x1 t0 g = concatF (FCons x1 t0 g) FEmpty) 
      by reflexivity; rewrite H1;
    apply lem_weaken_wfft; simpl;
    try apply wffe_uniqueF; try apply intersect_empty_r; intuition.
    destruct k; subst t0; assumption || apply WFFTKind; assumption.
  - assert (FConsT a k g = concatF (FConsT a k g) FEmpty) 
      by reflexivity; rewrite H1;
    inversion H0; apply lem_weaken_tv_wfft; simpl;
    try apply intersect_empty_r; try apply wffe_uniqueF; intuition.
  Qed.    