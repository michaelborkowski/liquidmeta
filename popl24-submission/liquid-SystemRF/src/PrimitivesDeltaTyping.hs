...
lem_delta_typ c v t_x t' p_c_txt' p_v_tx = case p_c_txt' of
  (TPrm _ _) -> TSub Empty (delta c v ? compat) (tsubBV v (ty' c)) p_cv_ty'cv
  ...
  (TSub {})  
    -> let (TSub _ _ s p_c_s _ k_t p_g_txt' p_s_t) 
               = lem_collapse_sub Empty (Prim c) (TFunc t_x t') WFEEmpty p_c_txt'
         in case p_s_t of
              (SFunc _ _ _ p_tx_in _ _ nms mk_p_ty'c_t') -> case p_c_s of
                (TPrm Empty _c) 
                      -> TSub Empty (delta c (v ? compat)) ty'cv p_cv_ty'cv
                    ...
                    where
                      ...
                      p_ty'cv_txty'  = lem_witness_sub Empty v t_x p_v_tx (ty' c) Star p_emp_exty'
                                                       WFEEmpty
                      p_txty'_extxt' = lem_subtype_in_exists Empty t_x (ty' c) 
 ...
 where
     p_cv_extxt'    = TApp Empty (Prim c) t_x t' p_c_txt' v p_v_tx
...
lem_deltaT_typ c k u' p_c_ku' t p_emp_t  = case p_c_ku' of
  ...
  (TSub {})
    -> let (TSub _ _ tyc p_c_tyc _ k_u p_emp_ku' p_tyc_u)
               = lem_collapse_sub Empty (Prim c) (TPoly k u') WFEEmpty p_c_ku'
         in case p_tyc_u of
              (SPoly _ _ tyc' _ nms mk_p_tyc'_u') -> case p_c_tyc of 
                (TPrm Empty _c) 
                      -> TSub Empty deltaT_c_t tyct p_ct_tyct (tsubBTV t u') Star
                              p_emp_u't p_tyct_u't
                    where
                      p_ct_u't    = TAppT Empty (Prim c) k u' p_c_ku' t p_emp_t
