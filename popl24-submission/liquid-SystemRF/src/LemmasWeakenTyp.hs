...
lem_weaken_typ_tvar1 g g' e t p_y_t@(TVar1 gg y t_y k_y p_gg_ty) x t_x
    = case g' of     -- env == concatE g (Cons y t_y g'')
        ...
        (Cons _y _ty g'') -> TVar1 (concatE (Cons x t_x g) g'') y t_y k_y p_gxg_ty        
          where
            p_gxg_ty = lem_weaken_wf g g'' t_y k_y p_gg_ty x t_x
-- (g; g' == _g, z:t_z) |- y : t_y
...
lem_weaken_typ_tvar2 g g' e t p_y_ty@(TVar2 gg y t_y p_gg_y_ty z t_z) x t_x
    = case g' of
        ...
        (Cons _z _tz g'') -> TVar2 (concatE (Cons x t_x g) g'') 
          ...
          where
            p_gxg_y_ty = lem_weaken_typ g g'' e t p_gg_y_ty x t_x
...
lem_weaken_typ_tvar3 g g' e t p_z_tz@(TVar3 gg z t_z p_z_t a' k_a') x t_x 
    = case g' of
        ...
        (ConsT _a' _ g'') -> TVar3 (concatE (Cons x t_x g) g'')
                                 ...
                                 t p_gxg_y_ty a' k_a' 
          where
            p_gxg_y_ty = lem_weaken_typ g g'' (FV z) t p_z_t x t_x
...
lem_weaken_typ_tapp g g' e t (TApp env e1 s s' p_env_e1_ss' e2 p_env_e2_s) x t_x 
    = TApp (concatE (Cons x t_x g) g') e1 s s' p_env'_e1_ss' e2 p_env'_e2_s
        where
          p_env'_e1_ss' = lem_weaken_typ g g' e1 (TFunc s s') p_env_e1_ss' x t_x
          p_env'_e2_s   = lem_weaken_typ g g' e2 s            p_env_e2_s   x t_x
...
lem_weaken_typ_tabst g g' e t p_e_t@(TAbsT env k1 e' t' nms mk_p_ag_e'_t') x t_x 
    = TAbsT (concatE (Cons x t_x g) g') k1 e' t' nms' mk_p_ax_e'_t'
        where
          ...
          mk_p_ax_e'_t' a = lem_weaken_typ g (ConsT a k1 g') 
...
lem_weaken_typ_tappt g g' e t p_e_t@(TAppT env e' k1 s p_e'_a1s t' p_env_t') x t_x 
    = TAppT (concatE (Cons x t_x g) g') e' k1 s p_env'_e'_a1s t' p_env'_t'
        where
          p_env'_e'_a1s = lem_weaken_typ g g' e' (TPoly k1 s) p_e'_a1s x t_x
          p_env'_t'     = lem_weaken_wf  g g' t' k1 p_env_t' x t_x
...
lem_weaken_typ_tlet g g' e t 
    ...
    = TLet (concatE (Cons x t_x g) g') e_y t_y p_env'_ey_ty 
        ...
        where
          p_env'_ey_ty       = lem_weaken_typ g g' e_y t_y p_env_ey_ty x t_x
          p_env'_t'          = lem_weaken_wf g g' t' k' p_env_t' x t_x
          {-@ mk_p_yenv'_e'_t' :: { y:Vname | NotElem y nms' }
                -> { pf:HasType | propOf pf == HasType (Cons y t_y (concatE (Cons x t_x g) g'))
                                                 (unbind y e') (unbindT y t') } @-}
          mk_p_yenv'_e'_t' y = lem_weaken_typ g (Cons y t_y g') (unbind y e') 
...
lem_weaken_typ_tann g g' e t p_e_t@(TAnn env e' _t p_env_e'_t) x t_x
    = TAnn (concatE (Cons x t_x g) g') e' t p_env'_e'_t
        where
          p_env'_e'_t = lem_weaken_typ g g' e' t p_env_e'_t x t_x
...
lem_weaken_typ_tsub g g' e t (TSub env _e s p_env_e_s _t k p_env_t p_env_s_t) x t_x
    = TSub (concatE (Cons x t_x g) g') e s p_env'_e_s t k p_env'_t p_env'_s_t 
        where
          p_env'_e_s = lem_weaken_typ     g g' e s p_env_e_s x t_x
          p_env'_t   = lem_weaken_wf      g g' t k p_env_t x t_x 
          p_env'_s_t = lem_weaken_subtype g g' s t p_env_s_t x t_x
...
lem_weaken_typ g g' e t p_y_t@(TVar1 {}) x_ t_x -- env == concatE g (Cons y t_y g'') 
    = lem_weaken_typ_tvar1 g g' e t p_y_t x_ t_x
lem_weaken_typ g g' e t p_y_ty@(TVar2 {}) x_ t_x
    = lem_weaken_typ_tvar2 g g' e t p_y_ty x_ t_x
lem_weaken_typ g g' e t p_y_ty@(TVar3 {}) x t_x 
    = lem_weaken_typ_tvar3 g g' e t p_y_ty x t_x
lem_weaken_typ g g' e t (TPrm _g c) x t_x = TPrm (concatE (Cons x t_x g) g') c
lem_weaken_typ g g' e t p_e_t@(TAbs {}) x t_x
    = lem_weaken_typ_tabs g g' e t p_e_t x t_x
lem_weaken_typ g g' e t p_e_t@(TApp {}) x t_x 
    = lem_weaken_typ_tapp g g' e t p_e_t x t_x
lem_weaken_typ g g' e t p_e_t@(TAbsT {}) x t_x 
    = lem_weaken_typ_tabst g g' e t p_e_t x t_x
lem_weaken_typ g g' e t p_e_t@(TAppT {}) x t_x 
    = lem_weaken_typ_tappt g g' e t p_e_t x t_x
lem_weaken_typ g g' e t p_e_t@(TLet {}) x t_x
    = lem_weaken_typ_tlet g g' e t p_e_t x t_x
lem_weaken_typ g g' e t p_e_t@(TAnn {}) x t_x
    = lem_weaken_typ_tann g g' e t p_e_t x t_x
lem_weaken_typ g g' e t p_e_t@(TSub {}) x t_x
    = lem_weaken_typ_tsub g g' e t p_e_t x t_x
...
lem_weaken_subtype_sfunc g g' ft1 ft2 
    ...
    = SFunc env' s1 s2 p_env'_s2_s1 t1 t2 nms' mk_p_wenv'_t1_t2
        where
          ...
          mk_p_wenv'_t1_t2 w = lem_weaken_subtype g (Cons w s2 g')
                                   (unbindT w t1) (unbindT w t2) (mk_p_wenv_t1_t2 w) x t_x
          nms'               = x:(unionEnv nms (concatE g g'))
...
lem_weaken_subtype_switn g g' t t2 (SWitn env v_z t_z p_env_vz_tz _t t' p_env_t_t'vz) x t_x
    = SWitn env' v_z t_z p_env'_vz_tz t t' p_env'_t_t'vz
        where
          env'          = concatE (Cons x t_x g) g'
          p_env'_vz_tz  = lem_weaken_typ g g' v_z t_z p_env_vz_tz x t_x
        ...
        where
          ...
          {-@ mk_p_zenv'_t_t' :: { z:Vname | NotElem z nms'}
                -> { pf:Subtype | propOf pf == Subtype (Cons z t_z (concatE (Cons x t_x g) g')) 
                                                 (unbindT z t) t' } @-}
          mk_p_zenv'_t_t' z = lem_weaken_subtype g (Cons z t_z g') 
...
lem_weaken_subtype_spoly g g' t1 t2 (SPoly env k' t1' t2' nms mk_p_a1g_t1'_t2') x t_x
    = SPoly (concatE (Cons x t_x g) g') k' t1' t2' nms' mk_p_a1x_t1'_t2'
        where
          ...
          mk_p_a1x_t1'_t2' a1 = lem_weaken_subtype g (ConsT a1 k' g') 
                                  (unbind_tvT a1 t1')  (unbind_tvT a1 t2') 
