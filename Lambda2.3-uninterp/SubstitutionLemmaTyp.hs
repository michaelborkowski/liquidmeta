...
{-@ lem_subst_typ_tvar1 :: g:Env -> { g':Env | Set_emp (Set_cap (binds g) (binds g')) } 
        ...
        -> {p'_e_t:HasType | propOf p'_e_t == HasType (concatE g (esubFV x v_x g')) 
                             ...
                             sizeOf p'_e_t <= (sizeOf p_e_t) + (sizeOf p_vx_tx) + 1 } 
         / [sizeOf p_e_t, 0] @-}
...
lem_subst_typ_tvar2 :: Env -> Env -> Vname -> Expr -> Type -> HasType -> WFEnv
                    -> Expr -> Type -> HasType -> HasType
lem_subst_typ_tvar2 g g' x v_x t_x p_vx_tx p_env_wf e t (TVar2 n env_ z _t p_z_t w_ t_w) 
...
lem_subst_typ_tvar3 :: Env -> Env -> Vname -> Expr -> Type -> HasType -> WFEnv
                    -> Expr -> Type -> HasType -> HasType
lem_subst_typ_tvar3 g g' x v_x t_x p_vx_tx p_env_wf e t (TVar3 n env_ z _t p_z_t a_ k_a) 
    = case g' of             -- g'' = Empty so x = w and p_z_t :: HasFType(g (FV z) t)
...
lem_subst_typ_tabs :: Env -> Env -> Vname -> Expr -> Type -> HasType -> WFEnv
                    -> Expr -> Type -> HasType -> HasType
lem_subst_typ_tabs g g' x v_x t_x p_vx_tx p_env_wf e t 
              ...
              -> { pf:HasType | propOf pf == 
                      HasType (Cons y (tsubFV x v_x t_z) (concatE g (esubFV x v_x g'))) 
                              (unbind y (subFV x v_x e')) (unbindT y (tsubFV x v_x t')) && 
                                sizeOf pf <= n + (sizeOf p_vx_tx) + 1 } @-}
        mk_p_yg'g_e'vx_t'vx y = lem_subst_typ g (Cons y t_z g') x v_x t_x p_vx_tx p_yenv_wf
...
lem_subst_typ_tapp :: Env -> Env -> Vname -> Expr -> Type -> HasType -> WFEnv
                    -> Expr -> Type -> HasType -> HasType
lem_subst_typ_tapp g g' x v_x t_x p_vx_tx p_env_wf e t 
              ...
              -> { pf:HasType | propOf pf ==
                      HasType (ConsT a' k (concatE g (esubFV x v_x g')))
                              (unbind_tv a' (subFV x v_x e')) (unbind_tvT a' (tsubFV x v_x t')) &&
                                sizeOf pf <= n + (sizeOf p_vx_tx) + 1 } @-}
        mk_p_a'env'_e'_t' a' = lem_subst_typ g (ConsT a' k g') x v_x t_x p_vx_tx p_a'env_wf 
...
lem_subst_typ_tappt g g' x v_x t_x p_vx_tx p_env_wf e t 
  ...
  = TAppT n' (concatE g (esubFV x v_x g')) (subFV x v_x e') k1 (tsubFV x v_x s) p_env'_e'_a1s
      ...
      where
        ...
        (WFEBind _ p_g_wf _ _ _ _) = p_xg_wf
        p_vx_er_tx    = lem_typing_hasftype g v_x t_x p_vx_tx p_g_wf
        p_env'_e'_a1s = lem_subst_typ g g' x v_x t_x p_vx_tx p_env_wf e' (TPoly k1 s) p_e'_a1s
        p_env'_t'     = lem_subst_wf  g g' x v_x t_x p_vx_er_tx t' k1 p_env_t'
        n'            = typSize p_env'_e'_a1s
...
lem_subst_typ_tlet :: Env -> Env -> Vname -> Expr -> Type -> HasType -> WFEnv
                    -> Expr -> Type -> HasType -> HasType
lem_subst_typ_tlet g g' x v_x t_x p_vx_tx p_env_wf e t 
        ...
        {-@ mk_p_yg'g_e'vx_tvx :: { y:Vname | NotElem y nms' } 
              -> { pf:HasType | propOf pf == 
                      HasType (Cons y (tsubFV x v_x t_z) (concatE g (esubFV x v_x g'))) 
                              (unbind y (subFV x v_x e')) (unbindT y (tsubFV x v_x t)) && 
                                sizeOf pf <= n + (sizeOf p_vx_tx) + 1 } @-}
        mk_p_yg'g_e'vx_tvx y = lem_subst_typ g (Cons y t_z g') x v_x t_x p_vx_tx p_yenv_wf 
                                         (unbind y e') (unbindT y t) (mk_p_yenv_e'_t y)
                                         ? lem_commute_subFV_unbind    x
                                               (v_x ? lem_typ_islc g v_x t_x p_vx_tx p_g_wf) y e'
                                         ? lem_commute_tsubFV_unbindT  x 
...
lem_subst_typ_tsub :: Env -> Env -> Vname -> Expr -> Type -> HasType -> WFEnv
                    -> Expr -> Type -> HasType -> HasType
lem_subst_typ_tsub g g' x v_x t_x p_vx_tx p_env_wf e t 
...
lem_subst_typ g g' x v_x t_x p_vx_tx p_env_wf e t p_e_t@(TVar2 {})
  = lem_subst_typ_tvar2 g g' x v_x t_x p_vx_tx p_env_wf e t p_e_t
lem_subst_typ g g' x v_x t_x p_vx_tx p_env_wf e t p_e_t@(TVar3 {})
...
lem_subst_typ g g' x v_x t_x p_vx_tx p_env_wf e t p_e_t@(TAbs {}) 
  = lem_subst_typ_tabs g g' x v_x t_x p_vx_tx p_env_wf e t p_e_t  
lem_subst_typ g g' x v_x t_x p_vx_tx p_env_wf e t p_e_t@(TApp {}) 
...
lem_subst_typ g g' x v_x t_x p_vx_tx p_env_wf e t p_e_t@(TLet {}) 
  = lem_subst_typ_tlet g g' x v_x t_x p_vx_tx p_env_wf e t p_e_t
lem_subst_typ g g' x v_x t_x p_vx_tx p_env_wf e t (TAnn n env_ e' t_ p_env_e'_t) 
  = TAnn (typSize p_g'g_e'_t) (concatE g (esubFV x v_x g')) 
         (subFV x v_x e') (tsubFV x v_x t) p_g'g_e'_t
      where
        p_g'g_e'_t = lem_subst_typ g g' x v_x t_x p_vx_tx p_env_wf e' t p_env_e'_t
lem_subst_typ g g' x v_x t_x p_vx_tx p_env_wf e t p_e_t@(TSub {}) 
...
{-@ lem_subst_sub_sbase :: g:Env -> { g':Env | Set_emp (Set_cap (binds g) (binds g')) } 
        ...
        -> {p'_s_t:Subtype | propOf p'_s_t == Subtype (concatE g (esubFV x v_x g')) 
                             ...
                             sizeOf p'_s_t <= max (sizeOf p'_s_t) (sizeOf p_vx_tx) } 
         / [ sizeOf p_s_t, 0 ] @-}
...
{-@ lem_subst_sub :: g:Env -> { g':Env | Set_emp (Set_cap (binds g) (binds g')) } 
        ...
        -> {p'_s_t:Subtype | propOf p'_s_t == Subtype (concatE g (esubFV x v_x g')) 
                                                         (tsubFV x v_x s) (tsubFV x v_x t)  &&
                             sizeOf p'_s_t <= (sizeOf p'_s_t) + (sizeOf p_vx_tx) + 1} 
         / [ sizeOf p_s_t, 1 ] @-}
lem_subst_sub :: Env -> Env -> Vname -> Expr -> Type -> HasType -> WFEnv
                    -> Type -> Kind -> WFType -> Type -> Kind -> WFType -> Subtype -> Subtype
lem_subst_sub g g' x v_x t_x p_vx_tx p_env_wf s k_s p_env_s t k_t p_env_t p_s_t = undefined
{-
lem_subst_sub g g' x v_x t_x p_vx_tx p_env_wf s k_s p_env_s t k_t p_env_t p_s_t@(SBase {}) 
...
lem_subst_sub g g' x v_x t_x p_vx_tx p_env_wf t k p_env_t t2 k2 p_env_t2 p_t_t2@(SWitn {}) 
    = lem_subst_sub_switn g g' x v_x t_x p_vx_tx p_env_wf t k p_env_t t2 k2 p_env_t2 p_t_t2
lem_subst_sub g g' x v_x t_x p_vx_tx p_env_wf t1 k1 p_env_t1 t' k' p_env_t' p_t1_t'@(SBind {}) 
