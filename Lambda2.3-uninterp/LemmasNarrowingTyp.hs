...
{-@ lem_narrow_typ_tvar1 :: g:Env -> { g':Env | Set_emp (Set_cap (binds g) (binds g')) } 
        ...
        -> {p'_e_t:HasType | propOf p'_e_t == HasType (concatE (Cons x s_x g) g') e t &&
                             sizeOf p'_e_t <= sizeOf p_e_t + sizeOf p_sx_tx + 1 } @-}
   ...
   = case g' of                                              -- t == self t'  (FV z) k'
      (Empty)           -> p_z_tx                            -- t == self t_x (FV z) k'
        where                                                -- z:t_x, G |- (FV z) : self t_x (FV z) k'
          ...
          n           = max (typSize p_z_self_sx) (subtypSize p_xg_sx_tx')
          p_z_tx      = TSub n (Cons x s_x g) (FV z) (self s_x (FV z) k') p_z_self_sx 
                             (self t_x (FV z) k') k' p_xsxg_t p_xg_sx_tx'
                             ? toProof ( subtypSize p_xg_sx_tx' =<= subtypSize p_sx_tx + 1)
                             ? toProof ( typSize p_z_self_sx === tdepth s_x 
                                       ? lem_subtypSize_lb g s_x t_x p_sx_tx
                                     =<= subtypSize p_sx_tx )
...
lem_narrow_typ g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf e _t (TVar2 n env' z_ t p_z_t w_ t_w) 
  = case g' of             -- g''=Emp so x=w and p_z_t :: HasType(g (FV z) t)
        (Empty)           -> TVar2 n  g z_ t p_z_t w_ s_x
        (Cons _w _tw g'') -> TVar2 n' (concatE (Cons x s_x g) g'') z t p'_z_t w t_w 
          where
...
lem_narrow_typ g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf e t (TVar3 n env_ z_ _t p_z_t a_ k_a) 
  = case g' of 
        (Empty)            -> impossible "x <> a"
        (ConsT _a _ka g'') -> TVar3 n' (concatE (Cons x s_x g) g'') z t p'_z_t a k_a
          where
            z      = z_ ? lem_in_env_concat (Cons x s_x g) g'' z_
                        ? lem_in_env_concat (Cons x t_x g) g'' z_
...
lem_narrow_typ g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf e t 
  ...
  = TAbs n' (concatE (Cons x s_x g) g') t_z k_z p_env'_tz e' t' nms' mk_p_yenv'_e'_t'
      where
...
lem_narrow_typ g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf e t 
  ...
  = TApp n' (concatE (Cons x s_x g) g') e' t_z t' p_env'_e'_tzt' e_z p_env'_ez_tz
      where
        ...
        n'             = n + subtypSize p_sx_tx + 1
lem_narrow_typ g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf e t 
               (TAbsT n env k e' t' nms mk_p_aenv_e'_t')
  = TAbsT n' (concatE (Cons x s_x g) g') k e' t' nms' mk_p_aenv'_e'_t'
      where
        ...
        n'                 = n + subtypSize p_sx_tx + 1
lem_narrow_typ g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf e t 
               (TAppT n env e' k s p_e'_as t' p_env_t') 
  = TAppT n' (concatE (Cons x s_x g) g') e' k s p_env'_e'_as t' p_env'_t'
      where
...
lem_narrow_typ g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf e t 
  ...
  = TLet n' (concatE (Cons x s_x g) g') e_z t_z p_env'_ez_tz e' t k p_env'_t nms' mk_p_yenv'_e'_t
      where
        {-@ mk_p_yenv'_e'_t :: { y:Vname | NotElem y nms' }
              -> { pf:HasType | propOf pf == HasType (Cons y t_z (concatE (Cons x s_x g) g'))
                                                     (unbind y e') (unbindT y t) &&
        ...
        nms'              = unionEnv nms (concatE (Cons x s_x g) g')
        n'                = n + subtypSize p_sx_tx + 1
lem_narrow_typ g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf e t (TAnn n env_ e' t_ p_env_e'_t) 
  = TAnn n' (concatE (Cons x s_x g) g') e' t p_env'_e'_t
      where
...
lem_narrow_typ g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf e t 
  ...
  = TSub n' (concatE (Cons x s_x g) g') e s p_env'_e_s t k p_env'_t p_env'_s_t
      where
...
lem_narrow_sub_sfunc g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf ty1 {-ky1 p_env_ty1-} ty2 {-ky2 p_env_ty2-}
  ...
  = SFunc n' (concatE (Cons x s_x g) g') s1 s2 p_env'_s2_s1 t1 t2 nms' mk_p_yenv'_t1_t2 
      where   
...
lem_narrow_sub_switn g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf t {-k p_env_t-} t2 {-k2 p_env_t2-}
  ...
  = SWitn n' (concatE (Cons x s_x g) g') v_z t_z p_env'_vz_tz t t' p_env'_t_t'vz  
      where
...
lem_narrow_sub_sbind g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf t1 {-k1 p_env_t1-} t' {-k' p_env_t'-}
  ...
  = SBind n' (concatE (Cons x s_x g) g') t_z t t' nms' mk_p_wenv'_t_t'
      where
...
lem_narrow_sub_spoly g g' x s_x k_sx p_g_sx t_x p_sx_tx p_g_wf t1 {-k1 p_env_t1-} t2 {-k2 p_env_t2-}
                     (SPoly n env k t1' t2' nms mk_p_env_t1'_t2') 
  = SPoly n' (concatE (Cons x s_x g) g') k t1' t2' nms' mk_p_env'_t1'_t2'
      where
