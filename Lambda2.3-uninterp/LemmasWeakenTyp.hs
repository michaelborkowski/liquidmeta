...
lem_weaken_typ_tvar1 g g' {-p_env_wf-} e t p_y_t@(TVar1 gg y t_y k_y p_gg_ty) x_ t_x
    = case g' of     -- env == concatE g (Cons y t_y g'')
        (Empty)           -> TVar2 n (Cons y t_y gg) y t p_y_t x t_x   
          where
...
lem_weaken_typ_tvar2 g g' {-p_env_wf-} e t p_y_ty@(TVar2 n gg y t_y p_gg_y_ty z t_z) x_ t_x
    = case g' of
        (Empty)           -> TVar2 (n+1) (concatE g g') y t_y p_y_ty x t_x 
          where
...
--            p_gg_ty = lem_typing_wf gg (FV y) t_y p_gg_y_ty p_gg_wf
        (Cons _z _tz g'') -> TVar2 (n+1) (concatE (Cons x t_x g) g'') 
...
lem_weaken_typ_tvar3 g g' {-p_env_wf-} e t p_z_tz@(TVar3 n gg z t_z p_z_t a' k_a') x_ t_x 
    = case g' of
        (Empty)           -> TVar2 (n+1) (concatE g g') z t_z p_z_tz x t_x
          where
...
--            p_gg_tz = lem_typing_wf gg (FV z) t_z p_z_t p_gg_wf
        (ConsT _a' _ g'') -> TVar3 (n+1) (concatE (Cons x t_x g) g'')
...
lem_weaken_typ_tabs g g' {-p_env_wf-} e t p_e_t@(TAbs n env t_y k_y p_gg'_ty e' t' nms mk_p_y_e'_t') x t_x
    = TAbs (n+1) (concatE (Cons x t_x g) g') t_y k_y p_gxg'_ty e' t' nms' mk_p_yx_e'_t'
        where
...
lem_weaken_typ_tapp g g' {-p_env_wf-} e t (TApp n env e1 s s' p_env_e1_ss' e2 p_env_e2_s) x t_x 
    = TApp (n+1) (concatE (Cons x t_x g) g') e1 s s' p_env'_e1_ss' e2 p_env'_e2_s
        where
...
lem_weaken_typ_tabst g g' {-p_env_wf-} e t 
                     p_e_t@(TAbsT n env k1 e' t' nms mk_p_ag_e'_t') x t_x 
    = TAbsT (n+1) (concatE (Cons x t_x g) g') k1 e' t' nms' mk_p_ax_e'_t'
        where
...
lem_weaken_typ_tappt g g' {-p_env_wf-} e t p_e_t@(TAppT n env e' k1 s p_e'_a1s t' p_env_t') x t_x 
    = TAppT (n+1) (concatE (Cons x t_x g) g') e' k1 s p_env'_e'_a1s t' p_env'_t'
        where
...
lem_weaken_typ_tlet g g' {-p_env_wf-} e t 
               p_e_t@(TLet n env e_y t_y p_env_ey_ty e' t' k' p_env_t' nms mk_p_yenv_e'_t') x t_x
    = TLet (n+1) (concatE (Cons x t_x g) g') e_y t_y p_env'_ey_ty 
...
lem_weaken_typ_tann g g' {-p_env_wf-} e t (TAnn n env e' _t p_env_e'_t) x t_x
    = TAnn (n+1) (concatE (Cons x t_x g) g') e' t p_env'_e'_t
        where
...
lem_weaken_typ_tsub g g' {-p_env_wf-} e t (TSub n env _e s p_env_e_s _t k p_env_t p_env_s_t) x t_x
    = TSub (n+1) (concatE (Cons x t_x g) g') e s p_env'_e_s t k p_env'_t p_env'_s_t 
        where
...
lem_weaken_subtype_sfunc g g' {-p_env_wf-} ft1 {-k1 p_env_ft1-} ft2 {-k2 p_env_ft2 -}
                       (SFunc n env s1 s2 p_env_s2_s1 t1 t2 nms mk_p_wenv_t1_t2) x t_x
...
lem_weaken_subtype_switn g g' {-p_env_wf-} t {-k p_env_t-} t2 {-k2 p_env_t2-}
                   (SWitn n env v_z t_z p_env_vz_tz _t t' p_env_t_t'vz) x t_x 
...
lem_weaken_subtype_sbind g g' {-p_env_wf-} t1 {-k1 p_env_t1-} t' {-k' p_env_t'-}
                   (SBind n env t_z t _t' nms mk_p_zenv_t_t') x t_x
...
lem_weaken_subtype_spoly g g' {-p_env_wf-} t1 {-Star p_env_t1-} t2 {-Star p_env_t2 -}
                         (SPoly n env k' t1' t2' nms mk_p_a1g_t1'_t2') x t_x
    = SPoly (n+1) (concatE (Cons x t_x g) g') k' t1' t2' nms' mk_p_a1x_t1'_t2'
        where
