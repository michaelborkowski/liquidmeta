...
--                     sizeOf pf <= 3*(xdepth t1)*(xdepth t2)*(xdepth t3)* max n n' } @-}
...
lem_sub_trans_switnbind g p_g_wf t k p_g_t t' {-k' p_g_t'-} t'' k'' p_g_t''
  ...
  = lem_sub_trans g p_g_wf t k p_g_t (tsubBV v_x s') {-k' p_g_s'vx -}
                  t'' k'' p_g_t'' p_g_t_s'vx p_g_s'vx_t''
      where
--        (WFExis _ _ k_sx p_g_sx _ k_s' nms' mk_p_wg_s') 
--                     = lem_wfexis_for_wf_texists g s_x s' k' p_g_t'
        w            = fresh_varT {-(union-} nms {-nms')-} g s'
        p_vx_er_sx   = lem_typing_hasftype g v_x s_x p_vx_sx p_g_wf
--        p_g_s'vx     = lem_subst_wf  g Empty w v_x s_x p_vx_er_sx (unbindT w s') k' (mk_p_wg_s' w)
--                                     ? lem_tsubFV_unbindT w v_x
--                                         (s' ? lem_free_subset_binds g t' k' p_g_t')
  ...
  True  -> SBind n' g s_x s t'' nms'' mk_p_yg_s_t''
      where
        ...
        mk_p_yg_s_t'' y = lem_sub_trans (Cons y s_x g) p_yg_wf (unbindT y s) k_s (mk_p_yg_s y)
                                        t' {-k' p_yg_t'-}
                                        t'' k'' p_yg_t'' (mk_p_yg_s_t' y) p_yg_t'_t''
          where
