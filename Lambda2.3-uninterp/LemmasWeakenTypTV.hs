...
{-@ lem_weaken_tv_typ_tvar1 :: g:Env -> { g':Env | Set_emp (Set_cap (binds g) (binds g')) }
        ...
        -> { p'_e_t:HasType | propOf p'_e_t == HasType (concatE (ConsT a k_a g) g') e t &&
                              sizeOf p'_e_t <= sizeOf p_e_t + 1 } / [ sizeOf p_e_t, 0 ] @-}
...
lem_weaken_tv_typ_tvar1 g g' {-p_env_wf-} e t p_y_t@(TVar1 gg y t_y k_y p_gg_ty) a k_a
    = case g' of     -- env == concatE g (Cons y t_y g'')
        (Empty)           -> TVar3 n (Cons y t_y gg) y t p_y_t a k_a 
          where
            n = typSize p_y_t
...
lem_weaken_tv_typ_tvar2 g g' {-p_env_wf-} e t p_y_ty@(TVar2 n gg y t_y p_gg_y_ty z t_z) a k_a
    = case g' of
        (Empty)           -> TVar3 (n+1) (concatE g g') y t_y p_y_ty a k_a
        (Cons _z _tz g'') -> TVar2 (n+1) (concatE (ConsT a k_a g) g'') 
...
lem_weaken_tv_typ_tvar3 g g' {-p_env_wf-} e t p_z_tz@(TVar3 n gg z t_z p_z_t a' k_a') a k_a
    = case g' of
        (Empty)           -> TVar3 (n+1) (concatE g g') z t_z p_z_tz a k_a
        (ConsT _a' _ g'') -> TVar3 (n+1) (concatE (ConsT a k_a g) g'')
...
lem_weaken_tv_typ_tabs g g' {-p_env_wf-} e t p_e_t@(TAbs n env t_y k_y p_gg'_ty e' t' nms mk_p_y_e'_t') a k_a
    = TAbs (n+1) (concatE (ConsT a k_a g) g') t_y k_y p_gag'_ty e' t'  nms' mk_p_ya_e'_t'
        where
            p_gag'_ty    = lem_weaken_tv_wf g g' t_y k_y p_gg'_ty a k_a
--            p_y''env_wf  = WFEBind env p_env_wf y'' t_y k_y p_gg'_tya
...
lem_weaken_tv_typ_tapp g g' {-p_env_wf-} e t (TApp n env e1 s s' p_env_e1_ss' e2 p_env_e2_s) a k_a
    = TApp (n+1) (concatE (ConsT a k_a g) g') e1 s s' p_env'_e1_ss' e2 p_env'_e2_s
        where
...
lem_weaken_tv_typ_tabst g g' {-p_env_wf-} e t 
                     p_e_t@(TAbsT n env k1 e' t' nms mk_p_a'g_e'_t') a k_a
    = TAbsT (n+1) (concatE (ConsT a k_a g) g') k1 e' t'  nms' mk_p_a'a_e'_t'
        where
...
lem_weaken_tv_typ_tappt g g' {-p_env_wf-} e t p_e_t@(TAppT n env e' k1 s p_e'_a1s t' p_env_t') a k_a
    = TAppT (n+1) (concatE (ConsT a k_a g) g') e' k1 s p_env'_e'_a1s t' p_env'_t'
        where
...
lem_weaken_tv_typ_tlet g g' {-p_env_wf-} e t 
               p_e_t@(TLet n env e_y t_y p_env_ey_ty e' t' k' p_env_t' nms mk_p_yenv_e'_t') a k_a
    = TLet (n+1) (concatE (ConsT a k_a g) g') e_y t_y p_env'_ey_ty 
...
lem_weaken_tv_typ_tann g g' {-p_env_wf-} e t (TAnn n env e' _t p_env_e'_t) a k_a
    = TAnn (n+1) (concatE (ConsT a k_a g) g') e' t p_env'_e'_t
        where
...
{-@ lem_weaken_tv_typ :: g:Env -> { g':Env | Set_emp (Set_cap (binds g) (binds g')) }
        ...
        -> { a:Vname | (not (in_env a g)) && (not (in_env a g')) } -> k_a:Kind
        -> { p'_e_t:HasType | propOf p'_e_t == HasType (concatE (ConsT a k_a g) g') e t &&
                              sizeOf p'_e_t <= sizeOf p_e_t + 1} / [ sizeOf p_e_t, 1 ] @-}
...
--        p_env_s    = lem_typing_wf         env e s p_e_s p_env_wf
        ...
        p_env'_t   = lem_weaken_tv_wf      g g' {-p_env_wf-} t k p_env_t a k_a
        p_env'_s_t = lem_weaken_tv_subtype g g' {-p_env_wf-} s {-Star p_env_s-} 
                                           t {-k p_env_t-} p_s_t a k_a
...
{-@ lem_weaken_tv_subtype_sbase :: g:Env -> { g':Env | Set_emp (Set_cap (binds g) (binds g')) }
      ...
      -> { p'_t_t':Subtype | propOf p'_t_t' == Subtype (concatE (ConsT a k_a g) g') t t' && 
                             sizeOf p'_t_t' <= sizeOf p_t_t' + 1 } / [ sizeOf p_t_t', 0 ] @-}
...
lem_weaken_tv_subtype_sbase g g' {-p_env_wf-} t {-k p_env_t-} t' {-k' p_env_t' -}
                       (SBase env b p1 p2 nms mk_pf_yenv_p1_p2) a k_a
...
lem_weaken_tv_subtype_sfunc g g' {-p_env_wf-} ft1 {-k1 p_env_ft1-} ft2 {-k2 p_env_ft2 -}
                       (SFunc n env s1 s2 p_env_s2_s1 t1 t2 nms mk_p_wenv_t1_t2) a k_a
    = SFunc (n+1) env' s1 s2 p_env'_s2_s1 t1 t2 nms' mk_p_wenv'_t1_t2
        where
          env'           = concatE (ConsT a k_a g) g'
...
lem_weaken_tv_subtype_switn g g' {-p_env_wf-} t {-k p_env_t-} t2 {-k2 p_env_t2-}
                   (SWitn n env v_z t_z p_env_vz_tz _t t' p_env_t_t'vz) a k_a
    = SWitn (n+1) env' v_z t_z p_env'_vz_tz t t' p_env'_t_t'vz
        where
          env'          = concatE (ConsT a k_a g) g'
...
lem_weaken_tv_subtype_sbind g g' {-p_env_wf-} t1 {-k1 p_env_t1-} t' {-k' p_env_t'-}
                   (SBind n env t_z t _t' nms mk_p_zenv_t_t') a k_a
    = SBind (n+1) env' t_z t t' nms' mk_p_zenv'_t_t'
        where
          env'           = concatE (ConsT a k_a g) g'
...
lem_weaken_tv_subtype_spoly g g' {-p_env_wf-} t1 {-Star p_env_t1-} t2 {-Star p_env_t2-}
                         (SPoly n env k' t1' t2' nms mk_p_a1g_t1'_t2') a k_a
    = SPoly (n+1) (concatE (ConsT a k_a g) g') k' t1' t2' nms' mk_p_a1a_t1'_t2'
        where
...
{-@ lem_weaken_tv_subtype :: g:Env -> { g':Env | Set_emp (Set_cap (binds g) (binds g')) }
      ...
      -> { a:Vname | not (in_env a g) && not (in_env a g') } -> k_a:Kind
      -> { p'_t_t':Subtype | propOf p'_t_t' == Subtype (concatE (ConsT a k_a g) g') t t' && 
                             sizeOf p'_t_t' <= sizeOf p_t_t' + 1 } / [ sizeOf p_t_t', 1 ] @-}
