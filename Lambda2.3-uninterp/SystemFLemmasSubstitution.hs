...
lem_subst_tv_pftyp :: FEnv -> FEnv -> Vname -> Type -> Kind -> WFFT -> WFFE
                          -> Preds -> PHasFType -> PHasFType
lem_subst_tv_pftyp g g' a t_a k_a p_ta_ka p_env_wf ps (PFTEmp  _)
  = PFTEmp  (concatF g (fesubFV a (erase t_a) g'))
lem_subst_tv_pftyp g g' a t_a k_a p_ta_ka p_env_wf ps (PFTCons _ p pf_p_bl ps' pf_ps'_bl)
  = PFTCons (concatF g (fesubFV a (erase t_a) g')) (subFTV a t_a p) pf_pta_bl 
            (psubFTV a t_a ps') pf_ps'ta_bl
      where
        pf_pta_bl   = lem_subst_tv_ftyp  g g' a t_a k_a p_ta_ka p_env_wf p (FTBasic TBool) pf_p_bl
        pf_ps'ta_bl = lem_subst_tv_pftyp g g' a t_a k_a p_ta_ka p_env_wf ps' pf_ps'_bl
