...
{-@ lem_subst_tv_ftyp :: g:FEnv -> { g':FEnv | Set_emp (Set_cap (bindsF g) (bindsF g')) }
        ...
        -> { t_a:UserType | Set_sub (free t_a) (vbindsF  g) && Set_sub (freeTV t_a) (tvbindsF g) &&
                            isLCT t_a }   -> k_a:Kind
        -> ProofOf(WFFT g (erase t_a) k_a) -> ProofOf(WFFE g) 
        -> e:Expr -> t:FType 
...
lem_subst_tv_ftyp :: FEnv -> FEnv -> Vname -> Type -> Kind -> WFFT -> WFFE
        -> Expr -> FType -> HasFType -> HasFType
lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e t (FTBC _env b) 
  = FTBC (concatF g (fesubFV a (erase t_a) g')) b ? lem_in_fenv_fesub g' a (erase t_a) a
lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e t (FTIC _env n)
  = FTIC (concatF g (fesubFV a (erase t_a) g')) n ? lem_in_fenv_fesub g' a (erase t_a) a
lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e t (FTVar1 _env z _t)
  = case g' of
      (FEmpty)         -> impossible "a <> z"
      (FCons _z _ g'') -> FTVar1 (concatF g (fesubFV a (erase t_a) g'')) 
                                 (z ? lem_in_env_concatF g g'' z) (ftsubFV a (erase t_a) t)
lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e t (FTVar2 _env z _t p_z_t w t_w)
  = case g' of
...
lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e t (FTAbs env_ t_z k_z p_env_tz e' t' nms mk_p_yenv_e'_t')
...
lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e t (FTApp env_ e' t_z t' p_env_e'_tzt' e_z p_env_ez_tz)
  = FTApp (concatF g (fesubFV a (erase t_a) g')) (subFTV a t_a e') (ftsubFV a (erase t_a) t_z) 
      ...
      where
        p_g'g_e'ta_tzt' = lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e' (FTFunc t_z t') p_env_e'_tzt'
        p_g'g_ezta_tz   = lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e_z t_z p_env_ez_tz
lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e t p_e_t@(FTAbsT env k' e' t' nms mk_p_a'env_e'_t')
...
lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e t (FTAppT env_ e' k' t' p_env_e'_a't' liqt p_env_er_liqt)
  = FTAppT (concatF g (fesubFV a (erase t_a) g')) (subFTV a t_a e') k' 
      ...
      where
        p_g'g_e'_a't' = lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e' (FTPoly k' t') p_env_e'_a't'
...
lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e t (FTLet env_ e_z t_z p_env_ez_tz e' t_ nms mk_p_yenv_e'_t)
...
lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e t (FTAnn env_ e' t_ liqt p_env_e'_t)
  = FTAnn (concatF g (fesubFV a (erase t_a) g')) (subFTV a t_a e') (ftsubFV a (erase t_a) t) 
      ...
      where
        ...
        p_g'g_e'_t = lem_subst_tv_ftyp g g' a t_a k_a p_g_ta p_g_wf e' t p_env_e'_t

{-@ lem_subst_pftyp :: g:FEnv -> { g':FEnv | Set_emp (Set_cap (bindsF g) (bindsF g')) }
        -> { x:Vname | (not (in_envF x g)) && not (in_envF x g') } -> v_x:Value
        -> t_x:FType -> ProofOf(HasFType g v_x t_x) -> ps:Preds
        -> ProofOf(PHasFType (concatF (FCons x t_x g) g') ps) 
        -> ProofOf(PHasFType (concatF g g') (psubFV x v_x ps)) @-} 
lem_subst_pftyp :: FEnv -> FEnv -> Vname -> Expr -> FType -> HasFType 
                        -> Preds -> PHasFType -> PHasFType
lem_subst_pftyp g g' x v_x t_x p_vx_tx ps (PFTEmp  _)
  = PFTEmp  (concatF g g')
lem_subst_pftyp g g' x v_x t_x p_vx_tx ps (PFTCons _ p pf_p_bl ps' pf_ps'_bl)
  = PFTCons (concatF g g') (subFV x v_x p) pf_pvx_bl (psubFV x v_x ps') pf_ps'vx_bl
      where
        pf_pvx_bl   = lem_subst_ftyp  g g' x v_x t_x p_vx_tx p (FTBasic TBool) pf_p_bl
        pf_ps'vx_bl = lem_subst_pftyp g g' x v_x t_x p_vx_tx ps' pf_ps'_bl
 
{-@ lem_subst_tv_pftyp :: g:FEnv -> { g':FEnv | Set_emp (Set_cap (bindsF g) (bindsF g')) }
        -> { a:Vname | (not (in_envF a g)) && not (in_envF a g') } 
        -> { t_a:UserType | Set_sub (free t_a) (vbindsF  g) && Set_sub (freeTV t_a) (tvbindsF g) &&
                            isLCT t_a }   -> k_a:Kind
        -> ProofOf(WFFT g (erase t_a) k_a) -> ProofOf(WFFE g) 
        -> ps:Preds -> ProofOf(PHasFType (concatF (FConsT a k_a g) g') ps)
        -> ProofOf(PHasFType (concatF g (fesubFV a (erase t_a) g')) (psubFTV a t_a ps)) @-}
lem_subst_tv_pftyp :: FEnv -> FEnv -> Vname -> Type -> Kind -> WFFT -> WFFE
                          -> Preds -> PHasFType -> PHasFType
lem_subst_tv_pftyp g g' a t_a k_a p_ta_ka p_g_wf ps (PFTEmp  _)
  = PFTEmp  (concatF g (fesubFV a (erase t_a) g'))
lem_subst_tv_pftyp g g' a t_a k_a p_ta_ka p_g_wf ps (PFTCons _ p pf_p_bl ps' pf_ps'_bl)
  = PFTCons (concatF g (fesubFV a (erase t_a) g')) (subFTV a t_a p) pf_pta_bl 
            (psubFTV a t_a ps') pf_ps'ta_bl
      where
        pf_pta_bl   = lem_subst_tv_ftyp  g g' a t_a k_a p_ta_ka p_g_wf p (FTBasic TBool) pf_p_bl
        pf_ps'ta_bl = lem_subst_tv_pftyp g g' a t_a k_a p_ta_ka p_g_wf ps' pf_ps'_bl
