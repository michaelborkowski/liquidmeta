...
--                               ? lem_fv_bound_in_env (erase_env g) v_x (erase t_x) p_vx_er_tx y_
...
lem_narrow_typ g g' x s_x p_g_sx t_x p_sx_tx p_env_wf e t 
               (TLet env_ e_z t_z p_env_ez_tz z e' t_ p_env_t y_ p_yenv_e'_t) 
  = undefined {-
  = TLet (concatE g (esubFV x v_x g')) (subFV x v_x e_z) (tsubFV x v_x t_z) p_g'g_ezvx_tzvx z
      ...
      where
        ...
        p_yg'g_e'vx_tvx  = lem_subst_typ g (Cons y t_z g') x v_x t_x p_vx_tx p_yenv_wf 
                                         ...
                                         ? lem_commute_subFV_subBV1 z (FV y) x v_x e'
                                         ? lem_commute_tsubFV_tsubBV1 z (FV y) x v_x t -}
lem_narrow_typ g g' x s_x p_g_sx t_x p_sx_tx p_env_wf e t (TAnn env_ e' t_ p_env_e'_t) 
  = TAnn (concatE (Cons x s_x g) g') e' t p_env'_e'_t
      where
        p_env'_e'_t = lem_narrow_typ g g' x s_x p_g_sx t_x p_sx_tx p_env_wf e' t p_env_e'_t
lem_narrow_typ g g' x s_x p_g_sx t_x p_sx_tx p_env_wf e t 
