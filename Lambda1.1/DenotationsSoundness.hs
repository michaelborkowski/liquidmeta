...
lem_denote_sound_typ_tlet :: Env -> Expr -> Type -> HasType -> WFEnv -> CSub 
                            -> DenotesEnv -> ValueDenoted
lem_denote_sound_typ_tlet g e t (TLet _g e_x t_x p_ex_tx x e' _t p_g_t y p_yg_e'_t) p_g_wf th den_g_th 
      ...
      (ValDen _ _ v_x_ ev_thex_vx den_thtx_vx) 
        -> ValDen (csubst th e) (ctsubst th t) v ev_the_v den_tht_v 
          where
            ...
            p_vx_er_tx         = get_ftyp_from_den (ctsubst th t_x) v_x den_thtx_vx
            ...
            (ValDen _ _ v ev_th'e'_v den_th't_v) = lem_denote_sound_typ (Cons y t_x g) (unbind x y e')
                                      (unbindT x y t) p_yg_e'_t
                                      (WFEBind g p_g_wf y t_x p_g_tx) th' den_g'_th'  
                                      ? lem_ctsubst_and_unbindT x y v_x (erase t_x) p_vx_er_tx th t
            ...
            ev_vxthe'_v        = AddStep (Let x v_x (csubst th e')) (subBV x v_x (csubst th e'))
                                   step_vxthe'_th'e' v (ev_th'e'_v 
                                      ? lem_csubst_and_unbind x y v_x (erase t_x) p_vx_er_tx th e')
...
lem_denote_sound_typ g e t p_e_t@(TLet _g e_x t_x p_ex_tx x e' _t p_g_t y p_yg_e'_t) p_g_wf th den_g_th
