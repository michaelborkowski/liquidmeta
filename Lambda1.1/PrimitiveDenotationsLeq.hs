...
module PrimitiveDenotationsLeq where
...
lem_den_leq :: Denotes
lem_den_leq = DFunc 1 (TRefn TInt 1 (Bc True)) t' (Prim Leq) (BTPrm BEmpty Leq) val_den_func
  where
...
lem_den_leqn n = DFunc 2 (TRefn TInt 2 (Bc True)) t'n (Prim (Leqn n)) (BTPrm BEmpty (Leqn n)) val_den_func
  where
    t'n = TRefn TBool 3 (App (App (Prim Eqv) (BV 3)) (App (App (Prim Leq) (Ic n)) (BV 2)) )
