...
data SubtypeStar where
    SubRefl :: Env -> Type -> WFType -> WFEnv -> SubtypeStar
    SubStep :: Env -> Type -> WFType -> Type -> Type -> Subtype -> SubtypeStar -> WFEnv -> SubtypeStar
{-@ data SubtypeStar where
...
{-@ lem_get_wftypeL_from_substar :: g:Env -> t:Type -> t':Type -> ProofOf(SubtypeStar g t t')
        -> { pf:WFType | propOf pf == WFType g t } @-} 
lem_get_wftypeL_from_substar :: Env -> Type -> Type -> SubtypeStar -> WFType
lem_get_wftypeL_from_substar g t _t (SubRefl _ _ p_g_t _)         = p_g_t
lem_get_wftypeL_from_substar g t t' (SubStep _ _ p_g_t _ _ _ _ _) = p_g_t

{-@ lem_get_wftypeR_from_substar :: g:Env -> t:Type -> t':Type -> ProofOf(SubtypeStar g t t')
        -> { pf:WFType | propOf pf == WFType g t' } @-} 
lem_get_wftypeR_from_substar :: Env -> Type -> Type -> SubtypeStar -> WFType
lem_get_wftypeR_from_substar g t _t (SubRefl _ _ p_g_t _) = p_g_t
lem_get_wftypeR_from_substar g t t' (SubStep _ _ _ t1 _ _ seq_t1_t' _)
...
lem_subtype_closed g _ t (SubRefl _g _t p_g_t p_g_wf)
  = lem_sub_refl g t p_g_t p_g_wf
lem_subtype_closed g s t (SubStep _g _s p_g_s s' _t p_s_s' seq_s'_t p_g_wf)
  = lem_sub_trans g p_g_wf s p_g_s s' p_g_s' t p_g_t p_s_s' p_s'_t
      where
        p_g_s' = lem_get_wftypeL_from_substar g s' t seq_s'_t
        p_g_t  = lem_get_wftypeR_from_substar g s' t seq_s'_t
...
lem_typ_lower_bound g e t p_wf_g (TSub _g _e s p_e_s _t p_g_t p_s_t)
  = LBForType g e t t' p_t'_t p_e_t'
      where
        ...
        p_t'_t = lem_sub_trans g p_wf_g t' p_g_t' s p_g_s t p_g_t p_t'_s p_s_t
lem_typ_lower_bound g e t p_wf_g p_e_t = LBForType g e t t p_t_t p_e_t
  where
    p_t_t = lem_sub_refl  g t p_g_t p_wf_g
    p_g_t = lem_typing_wf g e t p_e_t p_wf_g
