      ...
      (AppRed _ id ev_and_id _ v'_ ev_thq_v') -> case v'_ of
          ...
          _             -> impossible ("by lemma" ? lem_bool_values v' pf_v'_bl)
            where
              ...
              pf_v'_bl   = lemma_soundness thq v' ev_thq_v' (BTBase TBool) pf_thq_bl

{-@ lem_entails_and_commutes :: g:Env -> b:Base -> x:Vname -> p:Pred 
        -> { q:Pred | Set_sub (freeBV q) (Set_sng x) }
        -> { y:Vname | not (in_env y g) && not (Set_mem y (fv p)) && not (Set_mem y (fv q)) } 
        -> ProofOf(HasBType (BCons y (BTBase b) (erase_env g)) (unbind x y p) (BTBase TBool))
        -> ProofOf(HasBType (BCons y (BTBase b) (erase_env g)) (unbind x y q) (BTBase TBool))
...
lem_entails_and_commutes :: Env -> Base -> Vname -> Pred -> Pred -> Vname -> HasBType -> HasBType -> Entails
  ...
  where
    ...
    pandq = App (App (Prim And) p) q
                       ? toProof ( fv (App (App (Prim And) p) q)
                               === S.union (fv (App (Prim And) p)) (fv q)
                               === S.union (fv p) (fv q) )
                       ? lem_freeBV_emptyB (BCons y (BTBase b) (erase_env g)) 
                                           (unbind x y p) (BTBase TBool) pf_p_bl 
    g1       = (BCons y (BTBase b) (erase_env g))
    qandp = App (App (Prim And) q) p
                       ? toProof ( fv (App (App (Prim And) q) p)
                               === S.union (fv (App (Prim And) q)) (fv p)
                               === S.union (fv q) (fv p) )

{-@ lem_entails_trans :: g:Env -> b:Base -> x1:Vname -> p:Pred -> x2:Vname -> q:Pred -> x3:Vname -> r:Pred 
        -> { y:Vname | not (in_env y g) && not (Set_mem y (fv p)) && not (Set_mem y (fv q)) 
                                                                  && not (Set_mem y (fv r)) }
...
lem_entails_trans :: Env -> Base -> Vname -> Pred -> Vname -> Pred -> Vname -> Pred 
    ...
    where
      ...
      ev_thr_func' th den_gp_th = case den_gp_th of
        (DExt _g th' den_g_th' _y _bxp v den_thbxp_v) -> ev_thr_func th den_gq_th
          where
            p_v_b       = get_btyp_from_den (ctsubst th' (TRefn b x1 p)) v den_thbxp_v 
                                                   ? lem_ctsubst_refn th' b x1 p
            ev_thqv_tt  = ev_thq_func th den_gp_th ? lem_csubst_subBV x2 v (BTBase b) p_v_b th' q
...
{-@ lem_entails_change_bv :: g:Env -> b:Base -> x:Vname -> p:Pred -> x':Vname -> p':Pred
...
lem_entails_change_bv :: Env -> Base -> Vname -> Pred -> Vname -> Pred -> Vname -> Entails
  ...
  where
    ...
    ev_func th den_gp_th = case den_gp_th of   
      (DExt _g th' den_g_th' _y _bxp v den_th'bxp_v) -> ev_th'p'v_tt
        where
          p_v_b        = get_btyp_from_den (ctsubst th' (TRefn b x p)) v 
                                           (den_th'bxp_v ? lem_ctsubst_refn th' b x p)
          ev_th'pv_tt  = get_evals_from_drefn b x (csubst th' p) v 
                                           (den_th'bxp_v ? lem_ctsubst_refn th' b x p)
          ev_th'p'v_tt = ev_th'pv_tt ? lem_csubst_subBV x v (BTBase b) p_v_b th' p
                                     ? lem_subFV_unbind x y v p

{-@ lem_self_tt_sub_eql :: g:Env -> b:Base -> z:Vname -> z':Vname -> { x:Vname | not (in_env x g) } 
        -> ProofOf(Subtype (Cons x (TRefn b z (Bc True)) g) 
                     (self (TRefn b z (Bc True)) x) (TRefn b z' (App (App (Prim (equals b)) (BV z')) (FV x)))) @-} 
lem_self_tt_sub_eql :: Env -> Base -> Vname -> Vname -> Vname -> Subtype
lem_self_tt_sub_eql g b z z' x = SBase (Cons x t g) z b ttq z' eqx' w ent_ttq_eqx'
      where
        ...
        g2           = (BCons w (BTBase b) (erase_env (Cons x t g)))
        t            = TRefn b z (Bc True)
        pf_tt_bl     = BTBC g2 True
        pf_eqx_bl    = BTApp g2 (App (Prim (equals b)) (FV w)) (BTBase b) (BTBase TBool)
                         (BTApp g2 (Prim (equals b)) (BTBase b) (BTFunc (BTBase b) (BTBase TBool))
                                (BTPrm g2 (equals b)) (FV w) (BTVar1 (erase_env (Cons x t g)) w (BTBase b)))
                         (FV x) (BTVar2 (erase_env (Cons x t g)) x (BTBase b)
                                        (BTVar1 (erase_env g) x (BTBase b)) w (BTBase b)) 
