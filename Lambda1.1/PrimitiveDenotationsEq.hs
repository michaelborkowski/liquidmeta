...
module PrimitiveDenotationsEq where
...
lem_den_eq :: Denotes
lem_den_eq = DFunc 1 (TRefn TInt 1 (Bc True)) t' (Prim Eq) (BTPrm BEmpty Eq) val_den_func
  where
...
lem_den_eqn n = DFunc 2 (TRefn TInt 2 (Bc True)) t'n (Prim (Eqn n)) (BTPrm BEmpty (Eqn n)) val_den_func
  where
    t'n = TRefn TBool 3 (App (App (Prim Eqv) (BV 3)) (App (App (Prim Eq) (Ic n)) (BV 2)) )
