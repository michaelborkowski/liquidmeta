...
Lemma lem_den_len_zero_nil : forall (th:csub) (t:type) (ps:preds),
  loc_closed th -> substitutable th
    -> Denotes (TList (ctsubst th t) (cpsubst th ps)) Nil
    -> Denotes (TList (ctsubst th t) 
                      (cpsubst th (PCons (eq (Ic 0) (length t (BV 0))) ps))) Nil.
Proof. intros th t ps Hlc Hsub; intros; 
  ...
  rewrite <- lem_csubst_nil with th.
...
Proof. intros y v1 v2 th t ps Hlc Hsub Hc Hy Hfv1 Hftv1 Hfv Hftv Hlc2 Hfr Hfrv H.
  ...
  simpl; rewrite H. 
  rewrite <- (lem_csubst_nofv th (Cons v1 v2));
  try rewrite <- lem_csubst_subBV; unfold subBV; simpl;
  try rewrite <- lem_csubst_bc with th true;
  try apply lem_csubst_evals;
  pose proof (lem_subBV_lc (Cons v1 v2) v2); 
