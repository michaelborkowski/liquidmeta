...
Fixpoint esize (e : expr) : nat :=
    ...
    | Nil             => 0
    | (Cons e1 e2)    => 1 + (esize e1) + (esize e2)
...
Denotes (TList   t   ps) Nil :=
    HasFtype FEmpty Nil (erase (TList   t   ps)) /\
      PEvalsTrue (psubBV Nil ps);
Denotes (TList   t   ps) (Cons vH vT) :=
    HasFtype FEmpty (Cons vH vT) (erase (TList   t   ps)) /\
        isValue vH /\ isValue vT /\ 
        Denotes t vH /\ Denotes (TList t PEmpty) vT  /\
        PEvalsTrue (psubBV (Cons vH vT) ps);
