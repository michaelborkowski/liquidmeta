...
Proof. intros c t v isMeas noex mono p_emp_t Hval; 
  ...
  assert ( Prim Eq  = deltaT Eql (TRefn TInt PEmpty) pfZ ) as delTZ
    by (pose proof (deltaT_deltaT' Eql (TRefn TInt PEmpty) pfZ) as D;
        simpl in D; injection D; trivial ).
  - (* v = Nil *) 
    assert (isCompatM c Nil) as pf by (rewrite C; constructor).
    assert (deltaM c Nil pf = (Ic 0)) as del
                by ( pose proof (deltaM_deltaM' c Nil pf) as D; 
  ...
  apply lemma_list_values with v (erase t) in Hval as Hv.
  pose proof lemma_list_values.
