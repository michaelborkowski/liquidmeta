...
Proof. apply ( WFtype_ind 
  ...
  - (* WFRefn *) intro env; intros; destruct b eqn:B; simpl; simpl in H0;
    ...
    (* TBool / TInt / FTV a *) try ( 
      ...
      assert (concatF (FCons x (erase t_x) (erase_env g)) (FCons y (FTBasic b) (erase_env g'))
            = FCons y (FTBasic b) (erase_env (concatE (ECons x t_x g) g'))) as Henv'
  ...
  - (* WFFunc *) intros env t_y k_y; intros; subst env;
    ...
    assert (ECons y (tsubFV x v_x t_y) (concatE g (esubFV x v_x g')) 
              = concatE g (esubFV x v_x (ECons y t_y g')) ) as Henv
  ...
  - (* WFPoly *) intros env k; intros; subst env;
    ...
    assert (EConsT a' k (concatE g (esubFV x v_x g')) 
              = concatE g (esubFV x v_x (EConsT a' k g')) ) as Henv
      by reflexivity; rewrite Henv;
    rewrite <- lem_commute_tsubFV_unbind_tvT; try apply H0 with t_x;
    try apply intersect_names_add_intro_r; unfold in_env; simpl;
    try apply not_elem_names_add_intro; try split;
    try apply lem_ftyp_islc with (erase_env g) (erase t_x); intuition.
  - (* WFList *) intros env; intros; subst env.
    apply WFList with k_t; fold tsubFV. apply H0 with t_x; intuition.
  - (* WFListR *) intro env; intros; subst env;
    apply WFListR with (names_add x (union nms (binds (concatE g g')))); 
    fold tsubFV; fold psubFV; simpl in H0;
    try apply H0 with t_x;
    try (destruct ps; simpl; contradiction || discriminate); intros;
    try rewrite lem_erase_tsubFV;
    try assert (FCons y (FTList (erase t)) (erase_env (concatE g (esubFV x v_x g')))
            = concatF (erase_env g) (FCons y (FTList (erase t)) (erase_env g')) ) as Henv
        by (rewrite lem_erase_concat; rewrite lem_erase_esubFV; reflexivity || assumption);
    try rewrite Henv;
    try rewrite <- lem_commute_psubFV_unbindP;
    try apply lem_subst_pftyp with (erase t_x);
    try assert (concatF (FCons x (erase t_x) (erase_env g)) (FCons y (FTList (erase t)) (erase_env g'))
            = FCons y (FTList (erase t)) (erase_env (concatE (ECons x t_x g) g'))) as Henv'
        by (rewrite lem_erase_concat; reflexivity); 
    try rewrite Henv';
    try apply H2; simpl; try split; try unfold in_envF; simpl; 
    ...
    try apply lem_ftyp_islc with (erase_env g) (erase t_x); auto. 
...
Lemma lem_subst_wf : forall (g g':env) (x:vname) (v_x:expr) (t_x:type) (t:type) (k_t:kind),
  WFtype (concatE (ECons x t_x g) g' ) t k_t
                  ...
                  -> isValue v_x -> HasFtype (erase_env g) v_x (erase t_x)
                  -> WFtype (concatE g (esubFV x v_x g')) (tsubFV x v_x t) k_t.
Proof. intros; apply lem_subst_wf' with (concatE (ECons x t_x g) g') t_x;
...
Proof. intro g; induction g'; simpl; intros.
  ...
  - (* ECons  *) inversion H; apply WFEBind with k;
    ...
    trivial.
  - (* EConsT *) inversion H; apply WFEBindT;
    try apply IHg' with t_x;
    try apply not_elem_concat_intro; 
