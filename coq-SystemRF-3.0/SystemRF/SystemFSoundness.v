...
Proof. intros g e t p_e_t emp; induction p_e_t; simpl; subst g;
  ...
  - (* Cons eH eT *) destruct IHp_e_t1 as [H1|H1]; destruct IHp_e_t2 as [H2|H2];
      try reflexivity. 
      * (* Both vals *) left; constructor; assumption.
      * (* eH value *) right; destruct H2 as [eT' step]; exists (Cons eH eT');
        apply ECons2; assumption .
      * (* eT value *) right; destruct H1 as [eH' step]; exists (Cons eH' eT);
        apply ECons1; assumption .
      * (* no vals *) right; destruct H1 as [eH' step]; exists (Cons eH' eT);
        apply ECons1; assumption .
...
Proof. intros g e; induction e; intros t0 e' p_e_t emp st_e_e'; simpl; subst g;
  ...
  - (* FTCons *) inversion p_e_t; apply lemma_progress in H2 as Hprog;
    destruct Hprog as [Hval|Hstep].
    * (* e1 value *) apply lemma_progress in H4 as Hprog2;
      destruct Hprog2 as [Hval2|Hstep2];
      try (assert (isValue (Cons e1 e2)) by (constructor; assumption);
           apply lem_value_stuck in st_e_e'; assumption || contradiction);
      destruct Hstep2 as [e2' Hstep2].
      assert (e' = Cons e1 e2')
        by (apply lem_sem_det with (Cons e1 e2); try apply ECons2; assumption);
      subst e'; apply FTCons; assumption || apply IHe2; trivial.
    * (* e1 not val *) destruct Hstep as [e1' st_e1_e1']; assert (e' = Cons e1' e2)
        by (apply lem_sem_det with (Cons e1 e2); try apply ECons1; assumption);
  ...
  - (* FTSwitch *) inversion p_e_t; subst e1 e2 e3 t0;
    ...
    * (* e value *)  (* need lemma for list values! *)
      ...
      + (* e = Nil *) assert (e' = eN) 
          by (apply (lem_sem_det (Switch Nil eN eC)); try apply ESwitchN; assumption);
        subst e'; apply H5.
      + (* e = Cons *) assert (e' = (App (App eC e1) e2))
          by (apply (lem_sem_det (Switch (Cons e1 e2) eN eC)); inversion Hval;
              try apply ESwitchC; assumption);
        subst e'; inversion H3; apply FTApp with (FTList t); try apply H8;
        apply FTApp with t; assumption.
