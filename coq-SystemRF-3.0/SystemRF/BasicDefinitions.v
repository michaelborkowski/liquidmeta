...
Inductive isValue : expr -> Prop := (* can't use Prop here *)
    ...
    | val_Nil  :                isValue Nil
    | val_Cons : forall v1 v2,  isValue v1 -> isValue v2 -> isValue (Cons v1 v2).
...
Fixpoint isLC_at (j_x : index) (j_a : index) (e : expr) : Prop  :=
    ...
    | Nil            => True
    | (Cons e1 e2)   => isLC_at j_x j_a e1    /\ isLC_at j_x j_a e2 
...
Fixpoint open_at (j : index) (y : vname) (e : expr) : expr := 
    ...
    | Nil                => Nil
    | (Cons e1 e2)       => Cons (open_at j y e1) (open_at j y e2)    
...
Fixpoint open_tv_at (j : index) (a' : vname) (e0 : expr) : expr :=
    ...
    | Nil                          => Nil
    | (Cons e1 e2)                 => Cons      (open_tv_at j a' e1) (open_tv_at  j a' e2)
...
Fixpoint shift_at (j k : index) (e : expr) : expr := 
    ...
    | Nil                => Nil
    | (Cons e1 e2)       => Cons  (shift_at j k e1)  (shift_at j k e2)
    | (Switch e eN eC)   => Switch (shift_at j k e) (shift_at j k eN) (shift_at j k eC)
    | Error              => Error
    end
...
with shiftT_at (j k : index) (t0 : type) : type :=
    ...
    | (TList   t  ps) => TList   (shiftT_at j k t) (shiftP_at (j+1) k ps)
    end
...
Fixpoint subFV (x : vname) (v_x : expr) (e0 : expr) : expr :=
    ...
    | Nil                       => Nil
    | (Cons e1 e2)              => Cons (subFV x v_x e1) (subFV x v_x e2)
...
Fixpoint subFTV (a : vname) (t_a : type) (e0 : expr) : expr :=
    ...
    | Nil                       => Nil
    | (Cons e1 e2)              => Cons  (subFTV a t_a e1) (subFTV a t_a e2)
...
Fixpoint subBV_at (j : index) (v : expr) (e0 : expr) : expr :=
    ...
    | Nil                => Nil
    | (Cons e1 e2)       => Cons  (subBV_at j v e1)  (subBV_at j v e2)
...
Fixpoint subBTV_at (j : index) (t_a : type) (e0 : expr) : expr :=
    ...
    | Nil                          => Nil
    | (Cons e1 e2)                 => Cons      (subBTV_at j t_a e1) (subBTV_at j t_a e2)
