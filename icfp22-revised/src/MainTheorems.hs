...
thm_progress _e _t p_e_t@(TApp Empty e1 t_x t p_e1_txt e2 p_e2_tx) = case e1 of
...
thm_progress _e t p_e_t@(TAppT Empty e' k s p_e'_as t' p_emp_t') = case e' of
...
thm_progress _e _t (TLet Empty e1 tx p_e1_tx e2 t _ p_t y p_e2_t)
...
thm_preservation e t p_e_t@(TApp Empty (Prim c) t_x t' p_c_txt' e2 p_e2_tx) e' st_e_e'
  = case (thm_progress e2 t_x p_e2_tx) of
      ...
      Right (e2', st_e2_e2') -> Right (TApp Empty (Prim c) t_x t' p_c_txt' e2' p_e2'_tx)
        where
...
thm_preservation e t p_e_t@(TApp Empty (Lambda e1) t_x t' p_lam_txt' e2 p_e2_tx) e' st_e_e'
  = case (thm_progress e2 t_x p_e2_tx) of
      Left ()                -> Right p_e1e2_ex_t' ? lem_sem_det e e' st_e_e' 
                                                         (subBV e2 e1) (EAppAbs e1 e2)
        where
          (TAbs _ _ _ _ _ _ nms mk_p_e1_t')        
          ...
          p_e1e2_ex_t'        = TSub Empty (subBV e2 e1) (tsubBV e2 t') p_e1e2_t'e2
                                     (TExists t_x t') Star p_emp_ex_t' p_t'e2_ex_t'
      Right (e2', st_e2_e2') -> Right (TApp Empty (Lambda e1) t_x t' p_lam_txt' e2' p_e2'_tx)
        where
          Right p_e2'_tx      = thm_preservation e2 t_x p_e2_tx e2' st_e2_e2'  
                                    ? lem_sem_det e e' st_e_e' (App (Lambda e1) e2') 
                                                  (EApp2 e2 e2' st_e2_e2' (Lambda e1))
thm_preservation e t p_e_t@(TApp Empty e1 t_x t' p_e1_txt' e2 p_e2_tx) e' st_e_e' 
  = case (thm_progress e1 (TFunc t_x t') p_e1_txt') of
      Left ()                -> impossible ("by lemma" ? lemma_function_values e1 (erase t_x) (erase t')
                                                                                  p_e1_er_txt')
        where
          p_e1_er_txt'        = lem_typing_hasftype Empty e1 (TFunc t_x t') p_e1_txt' WFEEmpty
      Right (e1', st_e1_e1') -> Right (TApp Empty e1' t_x t' p_e1'_txt' e2 p_e2_tx)
        where
...
thm_preservation e t p_e_t@(TAppT Empty (Prim c) k s p_e1_as t'_ p_emp_t') e' st_e_e' 
...
thm_preservation e t (TAppT Empty (LambdaT k1 e2) k s p_e1_as t' p_emp_t') e' st_e_e' 
  = Right p_e2t'_st' -- subBTV a1 t' e2 :: tsubBTV a t' s      
          ? lem_sem_det e e' st_e_e' (subBTV t' e2) (EAppTAbs k1 e2 t') 
        where
          (TAbsT _ _ _ _ nms mk_p_e2_s)
...
thm_preservation e t (TAppT Empty e1 k s p_e1_as t' p_emp_t') e' st_e_e' 
  = case (thm_progress e1 (TPoly k s) p_e1_as) of
        ...
        Right (e1', st_e1_e1') -> Right (TAppT Empty e1' k s p_e1'_as t' p_emp_t')
          where
            Right p_e1'_as = thm_preservation e1 (TPoly k s) p_e1_as e1' st_e1_e1'
                              ? lem_sem_det e e' st_e_e' (AppT e1' t') (EAppT e1 e1' st_e1_e1' t')
thm_preservation e t p_e_t@(TLet Empty e_x t_x p_ex_tx e1 _t k p_emp_t nms mk_p_e1_t) e' st_e_e'
  = case (thm_progress e_x t_x p_ex_tx) of 
      ...
      Right (e_x', st_ex_ex') -> Right (TLet Empty e_x' t_x p_ex'_tx e1 t k p_emp_t nms mk_p_e1_t)
        where
