...
Proof. induction th; intros; simpl; try reflexivity;
  ...
  - rewrite <- IHth; simpl; try apply f_equal; 
    try apply lem_commute_subFV; trivial. 
    assert (subFV x v_x v_x0 = v_x0) by 
      (apply lem_subFV_notin; destruct H0; rewrite H0; intuition).
    rewrite <- H2 at 2. apply lem_commute_subFV.
    
    assert (~ Elem x (fv v_x0)).
    unfold closed in H0; destruct H0. rewrite H0. trivial. intuition.
    
    apply lem_com
  
  rewrite lem_commute_subFV.
