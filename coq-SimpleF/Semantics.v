...
Inductive isCompatT : prim -> ftype -> Set :=
    | isCptT_EqlB  :  isCompatT Eql  (FTBasic TBool)
    | isCptT_EqlZ  :  isCompatT Eql  (FTBasic TInt)
    | isCptT_LeqlB :  isCompatT Leql (FTBasic TBool)
    | isCptT_LeqlZ :  isCompatT Leql (FTBasic TInt).

Definition isCompatT' (c:prim) (t:ftype) : Prop := 
    match c, t with
...
Lemma compatT_prop_set : forall (c:prim) (t:ftype),
    isCompatT' c t -> isCompatT c t.
Proof. intros c t cpt'. destruct c; destruct t eqn:T;  
...
Definition deltaT (c : prim) (t : ftype) (pf : isCompatT c t) : expr :=
    match pf with
    | isCptT_EqlB   => Prim Eqv
    | isCptT_EqlZ   => Prim Eq
    | isCptT_LeqlB  => Prim Imp
    | isCptT_LeqlZ  => Prim Leq
    end.

Definition deltaT' (c : prim) (t : ftype) : option expr :=
    match c, t with
...
Lemma deltaT_deltaT' : forall (c : prim) (t : ftype) (pf : isCompatT c t),
    deltaT' c t =  Some (deltaT c t pf).
Proof. intros. destruct pf; simpl; reflexivity. Qed.

Lemma deltaT_pf_indep : forall (c : prim) (t : ftype) (pf pf' : isCompatT c t),
...
Inductive Step : expr -> expr -> Prop :=
    ...
    | EPrimT : forall (c : prim) (t : ftype) (pf : isCompatT c t),
          Step (AppT (Prim c) t) (deltaT c t pf) 
    | EAppT : forall (e e' : expr) (t : ftype),
          Step e e' -> Step (AppT e t) (AppT e' t)
    | EAppTAbs : forall (k : kind) (e : expr) (t : ftype),
          Step (AppT (LambdaT k e) t) (subBTV t e)
    ...
    | EAnn  : forall (e e' : expr) (t : ftype),
          Step e e' -> Step (Annot e t) (Annot e' t)
    | EAnnV : forall (v : expr) (t : ftype),
...
Lemma lemma_appT_many : forall (e e' : expr) (t : ftype), 
    EvalsTo e e' -> EvalsTo (AppT e t) (AppT e' t).
Proof. intros e e' t ev_e_e'. induction ev_e_e' as [e|e e1 e' st_ee1 ev_e1e' IH].
  - apply Refl.
...
Lemma lemma_annot_many : forall (e e':expr) (t:ftype),
...
Proof. intros e. induction e; intros e' e'' st_ee' st_ee''; inversion st_ee'.
  ...
  - (* e->e1 is EPrimT *) inversion st_ee''; subst e.
      ...
      * (* EAppT *) apply lem_value_stuck in H5; simpl in H5; contradiction.
      * (* EAppTAbs *) discriminate.
  - (* e->e1 is EAppT *) inversion st_ee''; subst e e0 t.
      * (* EPrimT *) apply lem_value_stuck in H2; simpl in H2. contradiction.
      * (* EAppT *) apply IHe with e'0 e'1 in H2 as H'. rewrite H'. reflexivity. assumption.
      * (* EAppTAbs *) apply lem_value_stuck in H2; simpl in H2; contradiction.
