...
Require Import SystemF.Semantics.
Require Import SystemF.SystemFWellFormedness.
Require Import SystemF.SystemFTyping.           
...
Lemma lem_prim_compatT_in_ftappt : forall (c:prim) (rt:ftype) (t:ftype),
    HasFtype FEmpty (AppT (Prim c) rt) t  ->  isCompatT' c rt.
Proof. intros c rt t p_crt_t. destruct c; inversion p_crt_t; inversion H2;
  subst k; apply lem_base_types in H4; 
  destruct rt eqn:Hrt; simpl in H4; try contradiction;
  simpl; trivial. Qed.
  
Lemma lem_deltaT_ftyp : forall (c:prim) (k:kind) (s:ftype) (t:ftype),
    HasFtype FEmpty (Prim c) (FTPoly k s) -> WFFT FEmpty t k
        -> exists e, Some e = deltaT' c t /\ HasFtype FEmpty e (ftsubBV t s).
Proof. intros c k s t p_c_aks p_emp_t; 
  destruct c; inversion p_c_aks; subst k;
  apply lem_base_types in p_emp_t as Ht;
  destruct t eqn:E; try contradiction;
