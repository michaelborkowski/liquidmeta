...
Fixpoint concatE (g g'0 : env) : env :=
    match g'0 with
    | Empty          => g
    | (ECons  x t g') => ECons  x t (concatE g g')
    | (EConsT a k g') => EConsT a k (concatE g g')
    end.
...
Lemma lem_vbinds_cons_concat : forall (g g' : env) (x : vname) (t_x : type),
    Subset (vbinds (concatE g g'))  (vbinds (concatE (ECons x t_x g) g')) /\
    Subset (vbinds (concatE (ECons x t_x g) g')) (names_add x (vbinds (concatE g g'))).
...
Lemma lem_tvbinds_cons_concat : forall (g g' : env) (x : vname) (t_x : type),
    Subset (tvbinds (concatE g g')) (tvbinds (concatE (ECons x t_x g) g')) /\
    Subset (tvbinds (concatE (ECons x t_x g) g')) (tvbinds (concatE g g')).
Proof. intros; induction g'; split; simpl;
...
Lemma lem_vbinds_consT_concat : forall (g g' : env) (a : vname) (k : kind),
    Subset (vbinds (concatE g g'))  (vbinds (concatE (EConsT a k g) g')) /\
    Subset (vbinds (concatE (EConsT a k g) g')) (vbinds (concatE g g')).
Proof. intros; induction g'; split; simpl;
  try apply subset_refl; try apply subset_add_both_intro; intuition. Qed.

Lemma lem_tvbinds_consT_concat : forall (g g' : env) (a : vname) (k : kind),
    Subset (tvbinds (concatE g g')) (tvbinds (concatE (EConsT a k g) g')) /\
    Subset (tvbinds (concatE (EConsT a k g) g')) (names_add a (tvbinds (concatE g g'))).
Proof. intros; induction g'; split; simpl;
  try apply subset_add_both_intro; try (apply subset_add_intro; apply subset_refl);
  try apply subset_refl; try apply IHg';
...
Proof. intro g; induction g'; simpl; intros.
  ...
  - (* EConsT *) repeat split; destruct H;
    apply IHg' in H0; destruct H0 as [Hg [Hg' Hi]];
    apply not_elem_concat_elim in H; destruct H;
    try apply intersect_names_add_intro_r; trivial.
  Qed.
...
Lemma lem_boundin_weaken : forall (x:vname) (t:type) (g g':env) (y:vname) (t_y:type),
  bound_in x t (concatE g g') -> bound_in x t (concatE (ECons y t_y g) g').
...
Lemma lem_boundin_weaken_tv : forall (x:vname) (t:type) (g g':env) (a:vname) (k:kind),
  bound_in x t (concatE g g') -> bound_in x t (concatE (EConsT a k g) g').
Proof. intros; apply lem_boundin_concat; simpl;
  apply lem_boundin_concat in H; intuition. Qed.
...
Lemma lem_tvboundin_weaken : forall (a:vname) (k:kind) (g g':env) (x:vname) (t:type),
  tv_bound_in a k (concatE g g') -> tv_bound_in a k (concatE (ECons x t g) g').
...
Lemma lem_tvboundin_weaken_tv : forall (a:vname) (k:kind) (g g':env) (a':vname) (k':kind),
  tv_bound_in a k (concatE g g') -> tv_bound_in a k (concatE (EConsT a' k' g) g').
Proof. intros; apply lem_tvboundin_concat; simpl;
...
Proof. induction t; intros; simpl.
  ...
  - (* TList *) apply f_equal; revert H; apply IHt.
  Qed.
...
Fixpoint esubFV (x:vname) (v:expr) (g0:env) : env :=
    match g0 with 
    | Empty            => Empty
    | (ECons  z t_z g) => ECons z (tsubFV x v t_z) (esubFV x v g)
    | (EConsT a1  k g) => EConsT a1 k              (esubFV x v g)
    end.
...
Fixpoint esubFTV (a:vname) (t_a:type) (g0:env) : env :=
    match g0 with 
    | Empty           => Empty
    | (ECons  z t_z g) => ECons z (tsubFTV a t_a t_z) (esubFTV a t_a g)
    | (EConsT a1  k g) => EConsT a1 k                 (esubFTV a t_a g)
    end.
