...
Inductive isCompatM : prim -> expr -> Set :=   (* can't use Prop here*)
    | isCptM_LNil : forall t, isCompatM Length (Nil t)
    | isCptM_LCons : forall t e es, 
          isCompatM Length es -> isCompatM Length (Cons t e es).
    
Fixpoint isCompatM' (c:prim) (v:expr) : Prop := 
    match c, v with
    | Length , Nil _         => True
    | Length , (Cons _ _ es) => isCompatM' Length es 
    | _      , _             => False
    end.
...
Fixpoint deltaM (c : prim) (v : expr) (pf : isCompatM c v) : expr :=
    match pf with
    | isCptM_LNil _           => Ic 0
    | isCptM_LCons _ _ es pf' => App (Prim Succ) (deltaM Length es pf')
    end.

Fixpoint deltaM' (c : prim) (v : expr) : option expr :=
    match c, v with
    | Length   , Nil _        => Some  (Ic 0)
    | Length  , (Cons _ _ es) => match (deltaM' Length es) with
...
Inductive Step : expr -> expr -> Prop :=
    ...
    | ECons1: forall (t : type) (e e' e1 : expr),
          Step e e' -> Step (Cons t e e1) (Cons t e' e1)
    | ECons2: forall (t : type) (e e' v : expr),
          isValue v -> Step e e' -> Step (Cons t v e) (Cons t v e')
    | ESwitch: forall (e e' eN eC : expr),
          Step e e' -> Step (Switch e eN eC) (Switch e' eN eC)
    | ESwitchN: forall (t : type) (eN eC : expr), Step (Switch (Nil t) eN eC) eN
    | ESwitchC: forall (t : type) (v1 v2 eN eC : expr),
          isValue v1 -> isValue v2 
            -> Step (Switch (Cons t v1 v2) eN eC) (App (App eC v1) v2).
...
Lemma lemma_cons_many : forall (t : type) (e e' v : expr),
    EvalsTo e e' -> EvalsTo (Cons t e v) (Cons t e' v).
Proof. intros t e e' v ev_e_e'. induction ev_e_e' as [e|e e1 e' st_ee1 ev_e1e' IH].
  - apply Refl.
  - apply AddStep with (Cons t e1 v). 
      * apply ECons1; apply st_ee1.
      * apply IH. 
  Qed. 
  
Lemma lemma_cons_many2 : forall (t : type) (v : expr) (e e' : expr), 
    isValue v -> EvalsTo e e' -> EvalsTo (Cons t v e) (Cons t v e').
Proof. intros t v e e' val ev_e_e'. induction ev_e_e' as [e|e e1 e' st_ee1 ev_e1e' IH].
  - apply Refl.
  - apply AddStep with (Cons t v e1).
      * apply ECons2; assumption.
      * apply IH.
  Qed.

Lemma lemma_cons_both_many : forall (t : type) (e e' : expr) (v v' : expr),
    isValue v -> isValue v' -> EvalsTo e v -> EvalsTo e' v'
        -> EvalsTo (Cons t e e') (Cons t v v').
Proof. intros t e e' v v' val val' ev_e_v ev_e'_v'. 
  apply lemma_evals_trans with (Cons t v e').
...
Proof. intros e. induction e; intros e' e'' st_ee' st_ee''; inversion st_ee'.
  ...
  - (* e->e1 is EIfF *) inversion st_ee''; subst e1 e' e''.
      ...
      * (* EIfT *) discriminate.
      * (* EIfF *) reflexivity.
  - (* e->e1 is ECons1 *) inversion st_ee''; subst t e e1 e2.
      * (* e->e2 is ECons1 *) apply IHe1 with e'0 e'1 in H3 as H'. rewrite H'; 
        reflexivity. assumption. 
      * (* e->e2 is ECons2 *) apply lem_value_stuck in H3; contradiction || assumption.
  - (* e->e1 is ECons2 *) inversion st_ee''; subst t e1 e2.
      * (* ECons1 *) apply lem_value_stuck in H9; contradiction || assumption.
