...
Definition index := nat. (* deBruijn indices *)
...
Inductive prim : Set :=
    ...
    | Length.        (* Polymorphic: For List[a] only *)
...
(* NEW in this version: explicit type instantiations for Nil and
    Cons to enable bidirectional checking *)
Inductive expr : Set :=  
    ...
    | Nil  (t : type)               (* [] *)
    | Cons (t : type) (e1 e2 : expr)(* e1 : e2 *)
    | Switch (e eN eC : expr)       (* case e of { Nil -> eN; Cons x1 x2 -> (eC x1) x2 } *)
    | Error
...
with type : Set :=
    ...
    | TList   (t : type)  (ps : preds)   (* refinements allowed so we can talk abt length *)
...
Combined Scheme syntax_mutind from 
    expr_mutind, type_mutind, preds_mutind.
...
Inductive isValue : expr -> Prop := (* can't use Prop here *)
    | val_Bc   : forall b,      isValue (Bc b)
    | val_Ic   : forall n,      isValue (Ic n)
    | val_Prm  : forall c,      isValue (Prim c)
    | val_Len  : forall t,      isValue (AppT (Prim Length) t)
    ...
    | val_Nil  : forall t,      isValue (Nil t)
    | val_Cons : forall t v1 v2, isValue v1 -> isValue v2 -> isValue (Cons t v1 v2).
...
Fixpoint isLC_at (j_x : index) (j_a : index) (e : expr) : Prop  :=
    ...
    | Nil t          => isLCT_at j_x j_a t 
    | (Cons t e1 e2) => isLCT_at j_x j_a t    /\ isLC_at j_x j_a e1 /\ isLC_at j_x j_a e2 
    | (Switch e eN eC) => isLC_at j_x j_a e   /\ isLC_at j_x j_a eN /\ isLC_at j_x j_a eC
    | Error          => True
    end
...
with isLCT_at (j_x : index) (j_a : index) (t0 : type) : Prop := 
    ...
    | (TList    t ps) => isLCT_at j_x j_a t   /\ isLCP_at (j_x+1) j_a ps
    end
...
Fixpoint open_at (j : index) (y : vname) (e : expr) : expr := 
    ...
    | Nil t              => Nil  (openT_at j y t)
    | (Cons t e1 e2)     => Cons (openT_at j y t) (open_at j y e1) (open_at j y e2)    
    | (Switch e eN eC)   => Switch (open_at j y e) (open_at j y eN) (open_at j y eC)   
    | Error              => Error
    end
...
with openT_at (j : index) (y : vname) (t0 : type) : type :=
    ...
    | (TList   t  ps) => TList   (openT_at j y t)   (openP_at (j+1) y ps)
    end
...
Fixpoint open_tv_at (j : index) (a' : vname) (e0 : expr) : expr :=
    ...
    | Nil t                        => Nil (open_tvT_at j a' t)
    | (Cons t e1 e2)               => Cons (open_tvT_at j a' t) (open_tv_at j a' e1) (open_tv_at  j a' e2)
    | (Switch e eN eC)             => Switch (open_tv_at j a' e) (open_tv_at j a' eN) (open_tv_at j a' eC)
    | Error                        => Error
    end
...
with open_tvT_at (j : index) (a' : vname) (t0 : type) : type  :=
    ...
    | (TList   t ps)    => TList    (open_tvT_at j a' t)  (open_tvP_at j a' ps)   
    end
...
(* Idea: (shift_at j e) only shifts indicies >= j because the lower indicies
    point inside of the affected expression  *)
Fixpoint shift_at (j : index) (e : expr) : expr := 
    match e with
    | (Bc b)             => Bc b
    | (Ic n)             => Ic n
    | (Prim c)           => Prim c
    | (BV i)             => if j <=? i then BV (i+1) else BV i 
    | (FV x)             => FV x
    | (Lambda e')        => Lambda (shift_at (j+1) e')
    | (App e1 e2)        => App   (shift_at j e1)  (shift_at j e2)
    | (LambdaT k0 e')    => LambdaT k0 (shift_at j e')  
    | (AppT e' t)        => AppT  (shift_at j e')  (shiftT_at j t)
    | (Let ex e')        => Let   (shift_at j ex)  (shift_at (j+1) e')
    | (Annot e' t)       => Annot (shift_at j e')  (shiftT_at j t)
    | (If e0 e1 e2)      => If (shift_at j e0) (shift_at j e1) (shift_at j e2)
    | Nil t              => Nil (shiftT_at j t)
    | (Cons t e1 e2)     => Cons (shiftT_at j t) (shift_at j e1)  (shift_at j e2)
    | (Switch e eN eC)   => Switch (shift_at j e) (shift_at j eN) (shift_at j eC)
    | Error              => Error
    end

with shiftT_at (j : index) (t0 : type) : type :=
    match t0 with
    | (TRefn b ps)    => TRefn b (shiftP_at (j+1) ps)
    | (TFunc   t_z t) => TFunc   (shiftT_at j t_z) (shiftT_at (j+1) t)
    | (TExists t_z t) => TExists (shiftT_at j t_z) (shiftT_at (j+1) t)
    | (TPoly   k0  t) => TPoly k0 (shiftT_at j t) 
    | (TList   t  ps) => TList   (shiftT_at j t) (shiftP_at (j+1) ps)
    end

with shiftP_at (j : index) (ps0 : preds) : preds  :=
    match ps0 with
    | PEmpty       => PEmpty
    | (PCons p ps) => PCons (shift_at j p) (shiftP_at j ps)
    end.

Definition shiftT (t : type)   : type  := shiftT_at 0 t.
Definition shiftP (ps : preds) : preds := shiftP_at 0 ps.
...
Fixpoint subFV (x : vname) (v_x : expr) (e0 : expr) : expr :=
    ...
    | Nil t                     => Nil (tsubFV x v_x t)  
    | (Cons t e1 e2)            => Cons (tsubFV x v_x t) (subFV x v_x e1) (subFV x v_x e2)
    | (Switch e eN eC)          => Switch (subFV x v_x e) (subFV x v_x eN) (subFV x v_x eC)
    | Error                     => Error
    end

with tsubFV (x : vname) (v_x : expr) (t0 : type) : type :=
    match t0 with
    | (TRefn  b ps)    => TRefn b   (psubFV x v_x ps)
    ...
    | (TList    t ps)  => TList    (tsubFV x v_x t)   (psubFV x v_x ps)
    end
...
Fixpoint subFTV (a : vname) (t_a : type) (e0 : expr) : expr :=
    ...
    | Nil t                     => Nil (tsubFTV a t_a t) 
    | (Cons t e1 e2)            => Cons (tsubFTV a t_a t)  (subFTV a t_a e1) (subFTV a t_a e2)
    | (Switch e eN eC)          => Switch (subFTV a t_a e) (subFTV a t_a eN) (subFTV a t_a eC)
    | Error                     => Error
    end
...
with tsubFTV (a : vname) (t_a : type) (t0 : type) : type  :=
    ...
    | (TList     t  ps)    => TList      (tsubFTV a t_a t)   (psubFTV a t_a ps) 
    end
...
Fixpoint subBV_at (j : index) (v : expr) (e0 : expr) : expr :=
    ...
    | Nil t              => Nil (tsubBV_at j v t)
    | (Cons t e1 e2)     => Cons (tsubBV_at j v t) (subBV_at j v e1)  (subBV_at j v e2)
    | (Switch e eN eC)   => Switch (subBV_at j v e)  (subBV_at j v eN) (subBV_at j v eC)
    | Error              => Error
    end
...
with tsubBV_at (j : index) (v_x : expr) (t0 : type) : type :=
    ...
    | (TList   t ps)  => TList   (tsubBV_at j v_x t) (psubBV_at (j+1) v_x ps)  
    end
...
Fixpoint subBTV_at (j : index) (t_a : type) (e0 : expr) : expr :=
    ...
    | Nil t                        => Nil (tsubBTV_at j t_a t)
    | (Cons t e1 e2)               => Cons (tsubBTV_at j t_a t) (subBTV_at j t_a e1) (subBTV_at j t_a e2)
    | (Switch e eN eC)             => Switch (subBTV_at j t_a e) (subBTV_at j t_a eN) (subBTV_at j t_a eC)
    | Error                        => Error
    end
...
with tsubBTV_at (j : index) (t_a : type) (t0 : type) : type :=
    ...
    | (TList   t ps)     => TList    (tsubBTV_at j t_a t)   (psubBTV_at j t_a ps)
    end
...
(* BARE TYPES: i.e. System F types. These still contain data types,  
     polymorphism and type variables, but all the refinements, 
     dependent arrow binders, and existentials have been erased. *)
...
Inductive ftype : Set :=
    ...
    Definition isFTList (t : ftype) : Prop :=
        match t with
        | (FTList _) => True
        | _          => False
        end.
...
Fixpoint openFT_at (j : index) (a' : vname) (t0 : ftype) : ftype :=
    ...
    | (FTList     t) => FTList (openFT_at j a' t)
    end.
...
Fixpoint ftsubFV (a : vname) (t_a : ftype) (t0 : ftype) : ftype :=
    ...
    | (FTList     t)  => FTList (ftsubFV a t_a t)
    end.
...
Fixpoint ftsubBV_at (j : index) (t_a : ftype) (t0 : ftype) : ftype :=
    ...
    | (FTList     t)  => FTList (ftsubBV_at j t_a t)
    end.
