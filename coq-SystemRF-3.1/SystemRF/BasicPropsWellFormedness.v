...
Proof. apply ( WFFT_ind 
  ...
  - (* WFFTBasic *) apply WFFTBasic; assumption.
  - (* WFFTFV *) apply WFFTFV;
  ...
  - (* WFFTList *) apply WFFTList with k_t; apply H0 with x t_x; trivial.
...
Proof. apply ( HasFtype_ind
  ...
  - (* FTNil *) apply FTNil with k; try apply lem_strengthen_wfft with x t_x; 
    pose proof (lem_vbinds_cons_concatF g g' x t_x) as Hv; destruct Hv as [Hv Hvx];
    pose proof (lem_tvbinds_cons_concatF g g' x t_x) as Htv; destruct Htv as [Htv Htvx];
    trivial.
    * apply subset_add_elim with x; 
      try apply subset_trans with (vbindsF (concatF (FCons x t_x g) g')); trivial.
    * apply subset_trans with (tvbindsF (concatF (FCons x t_x g) g')); trivial.
  - (* FTCons *) apply FTCons; try apply H5 with x t_x;
    try apply H7 with x t_x;
    simpl in H12; apply not_elem_union_elim in H12; destruct H12; 
...
Lemma lem_strengthen_hasftype_top : 
  forall (g:fenv) (x:vname) (t_x:ftype) (e:expr) (t:ftype),
    HasFtype (FCons x t_x g) e t -> ~ (in_envF x g) -> ~ Elem x (fv e)
                                 -> HasFtype g e t.
Proof. intros; assert (g = concatF g FEmpty) by reflexivity; rewrite H2. 
  apply lem_strengthen_hasftype with x t_x; simpl; try apply intersect_empty_r; auto. Qed.
...
Proof. intros g t k p_g_t ; induction p_g_t; simpl.
  ...
  - (* WFList *) apply WFFTList with k_t; apply IHp_g_t.
  - (* WFListR *) simpl in IHp_g_t; apply IHp_g_t.
...
Proof. intros g t k p_g_t; induction p_g_t. 
  ...
  - (* WFList *) simpl; apply IHp_g_t.
  - (* WFListR *) simpl; simpl in IHp_g_t.
    pose proof (fresh_varT_not_elem nms g (TList t ps)) as Hfv.
    set (z := (fresh_varT nms g (TList t ps))) in Hfv;
    destruct Hfv as [Hfv Hftv]; destruct Hftv as [Hftv Hnms];
    destruct Hnms as [Hnms Hg]; simpl in Hfv; simpl in Hftv.
    pose proof (lem_fvp_subset_bindsF (FCons z (FTList (erase t)) (erase_env g))
                                      (unbindP z ps) (H0 z Hnms)); 
    destruct H1 as [HfvP HftvP]; simpl in HfvP; simpl in HftvP; split;
    apply subset_union_intro_l; try apply IHp_g_t.
    * (* free *) rewrite vbinds_erase_env. 
      apply subset_add_elim with z;
      apply not_elem_union_elim in Hfv; try apply Hfv;
      apply subset_trans with (fvP (unbindP z ps));
      try apply fv_unbind_intro; apply HfvP.
    * (* freeTV *) rewrite tvbinds_erase_env.
      try ( apply subset_trans with (ftvP (unbindP z ps));
            try apply ftv_unbind_intro; apply HftvP).
...
Proof. intros; induction H; unfold isLCT; simpl; intuition.
  ...
  - (* WFPoly *) pose proof (fresh_not_elem nms);
    ...
    apply lem_islc_at_after_open_tv_at.
  - (* WFList *) unfold isLCT in IHWFtype; simpl in IHWFtype;
    destruct IHWFtype; assumption.
  - (* WFListR *) pose proof (fresh_not_elem nms);
    set (y := fresh nms) in H2; apply H1 in H2;  
    apply lem_pftyp_islcp in H2; revert H2;
    apply lem_islc_at_after_open_at.
  Qed.