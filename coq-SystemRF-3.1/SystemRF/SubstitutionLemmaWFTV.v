...
Proof. apply ( WFtype_ind 
  ...
  - (* WFRefn *) intro env; intros; destruct b eqn:B; simpl; simpl in H0;
    ...
    (* TBool / TInt / FTV a0 <> a *) try (
      ...
      assert (concatF (FConsT a k_a (erase_env g)) (FCons y (FTBasic b) (erase_env g'))
            = FCons y (FTBasic b) (erase_env (concatE (EConsT a k_a g) g'))) as Henv'
    ...
    * (* FTV a0 *) (* a = a0 *) apply lem_push_wf with (names_add a (union nms (binds (concatE g g')))); 
      ...
      assert (concatF (FConsT a k_a (erase_env g)) (FCons y (FTBasic (FTV a)) (erase_env g'))
            = FCons y (FTBasic (FTV a)) (erase_env (concatE (EConsT a k_a g) g'))) as Henv'
  ...
  - (* WFPoly *) intros env k0; intros; subst env.
    ...
    assert (EConsT a' k0 (concatE g (esubFTV a t_a g')) 
              = concatE g (esubFTV a t_a (EConsT a' k0 g')) ) as Henv
      by reflexivity; rewrite Henv.
  ...
  - (* WFList *) intros env; intros; subst env.
    apply WFList with k_t; fold tsubFTV. apply H0 with k_a; intuition.
  - (* WFListR *) intro env; intros; subst env;
    apply WFListR with (names_add a (union nms (binds (concatE g g')))); 
    fold tsubFTV; fold psubFTV; simpl in H0;
    try apply H0 with k_a; trivial;
    try (destruct ps; simpl; contradiction || discriminate); intros.
    apply not_elem_names_add_elim in H3;  destruct H3;
    apply not_elem_union_elim in H13;     destruct H13;
    apply not_elem_concat_elim in H14; try destruct H14.
    assert (FCons y (FTList (erase (tsubFTV a t_a t))) (erase_env (concatE g (esubFTV a t_a g')))
            = concatF (erase_env g) (fesubFV a (erase t_a) (FCons y (FTList (erase t)) (erase_env g'))) ) 
        as Henv by (rewrite lem_erase_concat; rewrite lem_erase_tsubFTV;
                    try rewrite lem_erase_esubFTV;
                    simpl; reflexivity || assumption);
    rewrite Henv; rewrite <- lem_commute_psubFTV_unbindP;
    try apply lem_subst_tv_pftyp with k_a;
    assert (concatF (FConsT a k_a (erase_env g)) (FCons y (FTList (erase t)) (erase_env g'))
            = FCons y (FTList (erase t)) (erase_env (concatE (EConsT a k_a g) g'))) as Henv'
        by (rewrite lem_erase_concat; reflexivity); try rewrite Henv';
    try apply H2; simpl; try split; try unfold in_envF; simpl;
    try apply unique_erase_env; try rewrite <- binds_erase_env;
    try apply intersect_names_add_intro_r; try rewrite <- binds_erase_env;
...
Lemma lem_subst_tv_wf : forall (g g':env) (a:vname) (t_a:type) (k_a:kind) (t:type) (k_t:kind),
   WFtype (concatE (EConsT a k_a g) g') t k_t 
                    ...
                    -> WFtype g t_a k_a -> WFFE (erase_env g)
                    -> WFtype (concatE g (esubFTV a t_a g')) (tsubFTV a t_a t) k_t.
Proof. intros; apply lem_subst_tv_wf'' with (concatE (EConsT a k_a g) g') k_a;
