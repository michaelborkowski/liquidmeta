...
Proof. apply ( HasFtype_ind 
  ...
  - (* FTNil *) apply FTNil with k; try apply lem_weaken_wfft; subst env;
    try (apply subset_trans with (vbindsF (concatF g g'));    
         apply lem_vbinds_cons_concatF || assumption);
    try (apply subset_trans with (tvbindsF (concatF g g'));    
         apply lem_tvbinds_cons_concatF || assumption); try assumption.
  - (* FTCons *) apply FTCons; try apply H5; try apply H7; subst env;
    try (apply subset_trans with (vbindsF (concatF g g'));    
         apply lem_vbinds_cons_concatF || assumption);
    try (apply subset_trans with (tvbindsF (concatF g g'));    
         apply lem_tvbinds_cons_concatF || assumption); trivial.
  - (* FTSwit *) apply FTSwit with t; apply H0 || apply H2 || apply H4;
    assumption .
  Qed.
...
Proof. apply ( HasFtype_ind 
  ...
  - (* FTAnn *) apply FTAnn; try apply H4; subst env;
    ...
    try assumption; try reflexivity.
  - (* FTIf *) apply FTIf; apply H0 || apply H2 || apply H4; assumption.
  - (* FTNil *) apply FTNil with k; try apply lem_weaken_tv_wfft; subst env;
    try (apply subset_trans with (vbindsF (concatF g g'));    
         apply lem_vbinds_consT_concatF || assumption);
    try (apply subset_trans with (tvbindsF (concatF g g'));    
         apply lem_tvbinds_consT_concatF || assumption); assumption.
  - (* FTCons *) apply FTCons; try apply H5; try apply H7; subst env; 
    try (apply subset_trans with (vbindsF (concatF g g'));    
         apply lem_vbinds_consT_concatF || assumption);
    try (apply subset_trans with (tvbindsF (concatF g g'));    
         apply lem_tvbinds_consT_concatF || assumption); trivial.
  - (* FTSwit *) apply FTSwit with t; apply H0 || apply H2 || apply H4;
    assumption .
  Qed.
