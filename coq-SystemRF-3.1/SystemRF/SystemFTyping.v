...
Definition refn_pred (c:prim) : expr := (* Set_emp (fv p) && Set_emp (ftv p) *)
    ...
    | Succ     => App (App (Prim Eq) 
                           (App (Prim Succ) (BV 1))) (BV 0)
    ...
    | Eql      => App (App (Prim Eqv) 
                           (App (App (AppT (Prim Eql)  (TRefn (BTV 0) PEmpty)) 
                                     (BV 2)) (BV 1))) (BV 0)
    | Length   => App (App (Prim Eq)
                           (App (AppT (Prim Length) (TRefn (BTV 0) PEmpty)) 
                                      (BV 1))) (BV 0)
    end.
...
Definition intype (c:prim ) : type := (* Set_emp (free t) && Set_emp (freeTV t) *)
    ...
    | Length  => TList (TRefn (BTV 0) PEmpty) PEmpty
    | _       => TRefn TInt    PEmpty
    end.
...
Definition ty' (c:prim) : type := (*Set_emp (free t) && Set_emp (freeTV t) *)
    ...
    | Succ     =>                             TRefn TInt  (PCons (refn_pred Succ) PEmpty)
    ...
    | Length   =>                               TRefn TInt  (PCons (refn_pred Length) PEmpty)
    end.
...
Inductive HasFtype : fenv -> expr -> ftype -> Prop := 
    ...
    | FTNil  : forall (g : fenv) (rt : type) (k:kind), 
          isMono rt -> noExists rt 
              -> Subset (free rt) (vbindsF g) 
              -> Subset (freeTV rt) (tvbindsF g) -> isLCT rt
              -> WFFT g (erase rt) k -> HasFtype g (Nil rt) (FTList (erase rt))
    | FTCons : forall (g : fenv) (eH eT : expr) (rt : type),
          isMono rt -> noExists rt 
              -> Subset (free rt) (vbindsF g) 
              -> Subset (freeTV rt) (tvbindsF g) -> isLCT rt
              -> HasFtype g eH (erase rt) -> HasFtype g eT (FTList (erase rt)) 
              -> HasFtype g (Cons rt eH eT) (FTList (erase rt))
    | FTSwit : forall (g : fenv) (e eN eC : expr) (t t' : ftype),
          HasFtype g e (FTList t) -> HasFtype g eN t'
              -> HasFtype g eC (FTFunc t (FTFunc (FTList t) t'))
              -> HasFtype g (Switch e eN eC) t'.
