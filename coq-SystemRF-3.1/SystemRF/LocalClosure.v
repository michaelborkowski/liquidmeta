...
Proof. apply (syntax_mutind
  ...
  - (* If e0 e1 e2 *) intros; destruct H4; destruct H5; repeat split;
      ...
      assumption.
  - (* Nil [t] *) intros t IHt j k j' k' j_le k_le. apply IHt.
      { apply j_le. } { apply k_le. }
  - (* Cons t e1 e2 *) intros t IHt e1 IHe1 e2 IHe2 j k j' k' j_le k_le Hjk. 
      destruct Hjk as [Ht [H1 H2]]. split; try split.
      { apply IHt with j k.  apply j_le. apply k_le. apply Ht. }
      { apply IHe1 with j k. apply j_le. apply k_le. apply H1. } 
      { apply IHe2 with j k. apply j_le. apply k_le. apply H2. }
  - (* Switch e eN eC *) intros. destruct H4 as [He [HeN HeC]]. repeat split.
      { apply H  with j k. all : assumption. }
      { apply H0 with j k. all : assumption. }
      { apply H1 with j k. all : assumption. }
  ...
  - (* TList t *) intros; destruct H3 as [Ht Hps]; split. 
      { apply H with j k; trivial. }
      { apply H0 with (j+1) k; try  apply Nat.add_le_mono_r; trivial. }
...
Proof. apply (syntax_mutind
  ...
  - (* Nil t *) intros; apply H; assumption.
  - (* Cons t e1 e2 *) intros; destruct H3 as [Ht [He1 He2]]. repeat split.
      { apply H.  all : assumption. } 
      { apply H0. all : assumption. }
      { apply H1. all : assumption. }
  - (* Switch e cs *) intros; destruct H3; destruct H4; repeat split; intuition.
  ...
  - (* TList t *) intros. destruct H2 as [Ht Hps]. split.
      { apply H. all: assumption. }
      { apply H0; assumption. }
...
Proof. apply (syntax_mutind
  ...
  (* 3 IH *) try ( intros; destruct H4; destruct H5; repeat split;
...
Proof. apply (syntax_mutind
  ...
  (* 3 IHs *) try ( intros; destruct H2; destruct H3; repeat split;
      apply H || apply H0 || apply H1; assumption ).
...
Proof. apply (syntax_mutind
  ...
  (* three IHs *) try ( intros; destruct H2; destruct H3; repeat split;
