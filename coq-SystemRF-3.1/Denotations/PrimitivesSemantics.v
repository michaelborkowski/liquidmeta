...
Lemma lem_step_length_nil : forall (t t0:type), 
    Step (App (AppT (Prim Length) t) (Nil t0)) (Ic 0).
Proof. intros t t0;
  assert (isCompatM Length (Nil t0)) as pfL
      by (apply isCptM_LNil).
  assert (Some (Ic 0) = Some (deltaM Length (Nil t0) pfL))
      by (rewrite deltaM_deltaM'; simpl; reflexivity);
  injection H as H; rewrite H; apply EPrimM; apply val_Nil. Qed. 
    
Lemma lem_step_length_cons : forall (t t0:type) (vH vT n': expr), 
  isValue vH -> isValue vT -> Step (App (AppT (Prim Length) t) vT) n' 
        -> Step (App (AppT (Prim Length) t) (Cons t0 vH vT)) 
                (App (Prim Succ) n').
Proof. intros; inversion H1;
  ...
  assert (isCompatM Length (Cons t0 vH vT)) as pfL
      by (apply isCptM_LCons; trivial).
  assert (Some (App (Prim Succ) n') = Some (deltaM Length (Cons t0 vH vT) pfL))
...
Lemma lemma_length_nil_semantics : forall (t t0:type) (p:expr),
    EvalsTo p (Nil t0) -> EvalsTo (App (AppT (Prim Length) t) p) (Ic 0).
Proof. intros; apply lemma_evals_trans with (App (AppT (Prim Length) t) (Nil t0)).
  - apply lemma_app_many2; constructor || trivial.
  - apply lem_step_evals; apply lem_step_length_nil.
  Qed.

Lemma lemma_length_cons_semantics : forall (t t0:type) (p vH vT:expr) (n:Z),
    isValue vH -> isValue vT 
        -> EvalsTo p (Cons t0 vH vT) -> EvalsTo (App (AppT (Prim Length) t) vT) (Ic n)
        -> EvalsTo (App (AppT (Prim Length) t) p) (Ic (1+n)).
Proof. intros; apply lemma_evals_trans with (App (AppT (Prim Length) t) (Cons t0 vH vT)).
  - apply lemma_app_many2; constructor || trivial.
...
Proof. intro t; induction v; intros val lst; simpl in lst; try contradiction.
  - exists 0%Z; apply lemma_length_nil_semantics with t0; apply Refl.
  - inversion val; destruct IHv2 as [n lenv2]; trivial.
    exists (1+n)%Z; apply lemma_length_cons_semantics with t0 v1 v2;
...
Lemma lemma_semantics_refn_length_nil : forall (t t0:type),
    EvalsTo (App (App (Prim Eq) (Ic BinNums.Z0)) 
                 (App (AppT (Prim Length) t) (Nil t0))) 
            (Bc true).
Proof. intros; apply lemma_evals_trans with (Bc (Z.eqb 0 0));
  try apply lemma_eq_semantics; try apply lemma_length_nil_semantics with t0;
  apply Refl. Qed.

Lemma lemma_semantics_refn_length_cons : forall (t t0:type) (vH vT:expr),
    isValue (Cons t0 vH vT) -> isList (Cons t0 vH vT)
      -> EvalsTo (App (App (Prim Eq) (App (Prim Succ) (App (AppT (Prim Length) t) vT))) 
                      (App (AppT (Prim Length) t) (Cons t0 vH vT))) 
                 (Bc true) .
Proof. intros; inversion H; simpl in H0;
  apply lemma_length_list_semantics with t vT in H0 as ev_lenvT; try apply H5;
  ...
  try apply lemma_length_cons_semantics with t0 vH vT;
  assert (1+n =? 1+n = true)%Z by (apply Z.eqb_eq; reflexivity); 
  try rewrite H6; try apply Refl; trivial.
  Qed.