...
Lemma lem_wfft_erase_ty : forall (g:fenv) (ds:defs) (c:prim),  
    WFFT g ds (erase_ty c) Star.
...
Lemma lem_bool_values : forall (ds:defs) (v:expr), 
    isValue v -> HasFtype FEmpty ds v (FTBasic TBool) ->  isBool v.
Proof. intros ds v val p_v_bl. inversion p_v_bl; subst v;
    (*try (inversion val; inversion H3).*)
    try contradiction.
    - (* v = Bc *) simpl; trivial.
    - (* v = Prim *) destruct c; simpl in H3; discriminate.
    - (* DC *) inversion val; try inversion H2. (* impossible somehow *)
    - (* App *)
    - (* AppT *)
    - (* Let *)
    - (* Annot *)
