...
lem_change_var_typ g x t_x g' p_env_wf e t (TVar1 _ z t') y  -- t == self t' z
    = case g' of 
        (Empty)           -> TVar1 g y (tsubFV x (FV y) t_x)  ? lem_free_bound_in_env g t_x p_g_tx x
...
lem_weaken_typ g g' p_env_wf e t p_y_t@(TVar1 gg y t_y) x_ t_x -- env == concatE g (Cons y t_y g'') = g;g' 
    = case g' of 
        (Empty)           -> TVar2 (Cons y t_y gg) (y ? lem_free_bound_in_env gg t_y p_gg_ty y) t p_y_t x t_x 
          where
...
{-@ lem_sub_refl :: g:Env -> t:Type -> { p_g_t:WFType | propOf p_g_t == WFType g t } -> ProofOf(WFEnv g) 
                          -> ProofOf(Subtype g t t) / [wftypSize p_g_t] @-}
...
lem_sub_refl g t p_g_t@(WFExis _g x t_x p_g_tx t' y p_yg_t') p_g_wf -- t = (\exists x:t_x. t')
    = SBind g x t_x t' (TExists x t_x t' ? lem_tfreeBV_empty g t p_g_t p_g_wf)
        ...
        where
          ...
          p_y_tx       = case t_x of 
            (TRefn b z p) -> TSub (Cons y t_x g) (FV y) (self t_x y) p_y_self_tx t_x p_yg_tx p_selftx_tx
              where
...
{-@ lem_change_bv_sub_func :: g:Env -> x:Vname -> t_x:Type -> t:Type -> x':Vname -> t':Type
        -> { y:Vname | not (in_env y g) && not (Set_mem y (free t)) && not (Set_mem y (free t')) 
                                        && unbindT x y t == unbindT x' y t' }
        -> ProofOf(WFType g t_x) -> ProofOf(WFType (Cons y t_x g) (unbindT x y t)) -> ProofOf(WFEnv g)
        -> ProofOf(Subtype g (TFunc x t_x t) (TFunc x' t_x t')) @-}
