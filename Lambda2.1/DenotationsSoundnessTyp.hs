...
{-@ lem_denote_exact_type :: th:CSub -> { x:Vname | not (in_csubst x th) }
        -> { t:Type | Set_sub (free t) (bindsC th) } -> w:Value 
...
lem_denote_exact_type th x (TRefn b z p) w den_tht_w = undefined {- case den_tht_w of
...
lem_denote_exact_type th x t@(TFunc z t_z t') w_ den_tht_w 
...
lem_denote_exact_type th x t@(TPoly a k_a t') w_ den_tht_w 
    = den_tht_w ? toProof ( ctsubst (CCons x w th) (self t (FV x))           
                        === ctsubst (CCons x w th) t
                        === ctsubst th (tsubFV x w t) 
                        === ctsubst th t )
