...
{-@ lem_subst_tv_sub_sbase_ftv :: g:Env -> { a:Vname | (not (in_env a g)) } -> k_a:Kind
      -> { g':Env | Set_emp (Set_cap (binds g) (binds g')) && not (in_env a g') 
                    && Set_emp (Set_cap (binds (ConsT a k_a g)) (binds g'))} 
      -> b':Basic -> z:RVname -> q:Pred -> ProofOf(WFType g (TRefn b' z q) k_a) 
...
lem_subst_tv_sub_sbase_ftv :: Env -> Vname -> Kind -> Env -> Basic -> RVname -> Expr -> WFType -> WFEnv
      -> RVname -> Expr -> Kind -> WFType -> RVname -> Expr -> Kind -> WFType -> Subtype -> Subtype
lem_subst_tv_sub_sbase_ftv g a k_a g' b' z q p_g_ta p_env_wf z1 p1 k_s p_env_s z2 p2 k_t p_env_t 
  ...
  = SBase (concatE g (esubFTV a t_a g')) z b' (strengthen (subFTV a t_a p1) q)
      ...
      where 
        t_a              = TRefn b' z q
        s                = TRefn (FTV a) z1 p1 
        y                = y_  ? lem_in_env_esubFTV g' a (TRefn b' z q) y_ 
                               ? lem_in_env_concat g  g' y_ 
                               ? lem_in_env_concat (ConsT a k_a g) g' y_
                               ? lem_in_env_concat g  (esubFTV a t_a g') y_ 
{-                               ? lem_free_bound_in_env g t_a k_a p_g_ta 
                                     (y_ ? lem_in_env_concat (ConsT a k_a g) g' y_)-}
        ...
        ent_yenv'_p2ta   = lem_subst_tv_ent g (Cons y s g') a t_a k_a p_g_ta p_yenv_wf
                                            (unbind 0 y p2 ? free_sub) (ent_yenv_p2 ? cons_yenv) ? esub_pf
...
{-        ent_yenv'_q      = snd ( lem_entails_unstrengthen env' p_er_env'_wf  b' z
                                         (strengthen (subFTV a t_a p1) q) k_s p_env'_t1
                                         (subFTV a t_a p1) q k_s p_env'_t1 y ent_yenv'_p1taq )-}
        ...
        bin_es           =   binds (esubFTV a (TRefn b' z q) (Cons y (TRefn (FTV a) z1 p1) g'))
                         === binds (Cons y (TRefn (FTV a) z1 p1) g')
        free_sub         = lem_free_subset_binds (concatE (ConsT a k_a g) g') (TRefn (FTV a) z2 p2) k_t p_env_t
        cons_yenv        = Cons y s env === Cons y s (concatE (ConsT a k_a g) g') ? bin_es
                                        === concatE (ConsT a k_a g) (Cons y s g')
        esub_pf          =   concatE g (esubFTV a (TRefn b' z q) (Cons y (TRefn (FTV a) z1 p1) g') ? bin_es)
                         === concatE g (Cons y (tsubFTV a (TRefn b' z q) (TRefn (FTV a) z1 p1)) 
                                               (esubFTV a (TRefn b' z q) g'))
                         === Cons y (tsubFTV a (TRefn b' z q) (TRefn (FTV a) z1 p1)) 
