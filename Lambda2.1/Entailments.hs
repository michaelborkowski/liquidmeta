...
{-@ lem_entails_elimination :: g:Env -> b:Basic -> x:RVname -> p:Term
        -> { q:Term | Set_sub (freeBV q) (Set_sng 0) }
        ...
        -> ProofOf(Entails (Cons y (TRefn b x (App (App (Prim Conj) p) q)) g) (unbind 0 y p)) @-}
lem_entails_elimination :: Env -> Basic -> RVname -> Expr -> Expr -> Vname 
                               -> HasFType -> HasFType -> Entails
lem_entails_elimination g b x p q y pf_p_bl pf_q_bl = EntPred (Cons y t1 g) (unbind 0 y p) ev_func
  where {- 1 undefined -}
    ...
    ev_func th den_g1_th = case den_g1_th of
      (DExt _g th' den_g_th' _y _t1 th'y den_th't1_th'y) ->
            fst (lem_implies_elimination (Cons y t1 g) th den_g1_th (unbind 0 y p) (unbind 0 y q) 
                     pf_p_bl pf_q_bl ( ev_thp'_tt 
               {-? toProof ( subBV x th'y (csubst th' pandq)-}
                           ? lem_csubst_subBV 0 th'y s {-(FTBasic b)-} p_th'y_s {-p_th'y_b-} th' pandq
        ...
        where
          {-@ ev_thp'_tt :: ProofOf(EvalsTo (subBV 0 th'y (csubst th' (App (App (Prim Conj) p) q))) 
                                            (Bc True)) @-}
          {-@ p_th'y_s :: ProofOf(HasFType FEmpty th'y s) @-}
          ev_thp'_tt    = undefined {-get_evals_from_drefn b x (csubst th' pandq) th'y (den_th't1_th'y
                            ? lem_ctsubst_refn th' b x pandq) -- (unbind x y pandq)-}
          (s, p_th'y_s) = get_ftyp_from_den (ctsubst th' t1) th'y den_th't1_th'y
