...
{-@ lem_remove_tvar_denote_env :: g:Env -> { a:Vname | not (in_env a g) } -> k_a:Kind
       ...
       -> ProofOf(WFEnv (concatE g g'))
       -> th:CSubst -> ProofOf(DenotesEnv (concatE (ConsT a k_a g) g') th)
       -> ProofOf(DenotesEnv (concatE g g') (remove_fromCS th a )) @-}
lem_remove_tvar_denote_env :: Env -> Vname -> Kind -> Env -> WFEnv -> CSubst 
                                 -> DenotesEnv -> DenotesEnv
...
{-@ lem_csubst_hasbtype' :: g:Env -> e:Expr -> t:Type -> ProofOf(HasFType (erase_env g) e (erase t))
        -> th:CSubst -> ProofOf(DenotesEnv g th) -> ProofOf(HasFType FEmpty (csubst th e) (erase t)) @-} 
lem_csubst_hasbtype' :: Env -> Expr -> Type -> HasFType -> CSubst -> DenotesEnv -> HasFType
