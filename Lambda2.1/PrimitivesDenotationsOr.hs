...
lem_den_or :: Denotes
lem_den_or = DFunc 1 (TRefn TBool Z (Bc True)) (ty' Or) (Prim Or) (FTPrm FEmpty Or) val_den_func_or
...
{-@ den_t't_lamtt :: ProofOf(Denotes (tsubBV 1 (Bc True) (ty' Or)) (Lambda 1 (Bc True))) @-}
den_t't_lamtt :: Denotes
den_t't_lamtt = DFunc 2 (TRefn TBool Z (Bc True)) t't (Lambda 1 (Bc True))
                      ...
                      val_den_func3
  where
    t't = tsubBV 1 (Bc True) (TRefn TBool Z (refn_pred Or))
...
val_den_func3 v_x den_tx_vx = case v_x of
  ...
  where
    ...
    {-@ or_ev_prt'''t_tt :: ProofOf(EvalsTo (subBV 0 (Bc True) p'''t) (Bc True)) @-}
    or_ev_prt'''t_tt = reduce_or_tt True True
    den_t'''f_tt = DRefn TBool Z p'''f (Bc True) (FTBC FEmpty True) or_ev_prt'''f_tt
    {-@ or_ev_prt'''f_tt :: ProofOf(EvalsTo (subBV 0 (Bc True) p'''f) (Bc True)) @-}
    or_ev_prt'''f_tt = reduce_or_tt True False
    p'''t = subBV 2 (Bc True)  (subBV 1 (Bc True) (refn_pred Or))
    p'''f = subBV 2 (Bc False)  (subBV 1 (Bc True) (refn_pred Or))

{-@ den_t'f_id :: ProofOf(Denotes (tsubBV 1 (Bc False) (ty' Or)) (Lambda 1 (BV 1))) @-}
den_t'f_id :: Denotes
den_t'f_id = DFunc 2 (TRefn TBool Z (Bc True)) t'f (Lambda 1 (BV 1)) 
                   ...
                   val_den_func2
  where
    t'f = tsubBV 1 (Bc False) (TRefn TBool Z (refn_pred Or)) 
...
val_den_func2 v_x den_tx_vx = case v_x of 
  ...
  where
    ...
    {-@ or_ev_prt''f_ff :: ProofOf(EvalsTo (subBV 0 (Bc False) p''f) (Bc True)) @-}
    or_ev_prt''f_ff = reduce_or_tt False False
    p''t = subBV 2 (Bc True)  (subBV 1 (Bc False) (refn_pred Or))
    p''f = subBV 2 (Bc False) (subBV 1 (Bc False) (refn_pred Or))
