...
lem_den_eq :: Denotes
lem_den_eq = DFunc 1 (TRefn TInt Z (Bc True)) t' (Prim Eq) (FTPrm FEmpty Eq) val_den_func
  where
    ...
    val_den_func v_x den_tx_vx = case v_x of 
      (Ic n) -> ValDen (App (Prim Eq) (Ic n)) (tsubBV 1 (Ic n) t') (Prim (Eqn n))
        ...
        where
          t'n = TRefn TBool Z (App (App (Prim Eqv) (BV 0)) (App (App (Prim Eq) (Ic n)) (BV 2)) )
          den_t'n_eqn = DFunc 2 (TRefn TInt Z (Bc True)) t'n (Prim (Eqn n)) 
          ...
          val_den_func2 v_x den_tx_vx = case v_x of 
            (Ic m) -> ValDen (App (Prim (Eqn n)) (Ic m)) (tsubBV 2 (Ic m) t'n) (Bc (n == m))
              ...
              where
                t'nm = TRefn TBool Z (App (App (Prim Eqv) (BV 0)) (App (App (Prim Eq) (Ic n)) (Ic m)) )
                den_t'nm_eq = DRefn TBool Z (App (App (Prim Eqv) (BV 0)) (App (App (Prim Eq) (Ic n)) (Ic m)))
...
lem_den_eqn n = DFunc 2 (TRefn TInt Z (Bc True)) t'n (Prim (Eqn n)) (FTPrm FEmpty (Eqn n)) val_den_func
  where
    t'n = TRefn TBool Z (App (App (Prim Eqv) (BV 0)) (App (App (Prim Eq) (Ic n)) (BV 2)) )
    {-@ val_den_func :: v_x:Value -> ProofOf(Denotes (TRefn TInt Z (Bc True)) v_x)
                                  -> ProofOf(ValueDenoted (App (Prim (Eqn n)) v_x) (tsubBV 2 v_x t'n)) @-}
