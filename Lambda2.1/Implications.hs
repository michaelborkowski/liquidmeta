...
lem_implies_elimination g th den_g_th p q pf_p_bl pf_q_bl ev_thpq_tt_ 
  = let thp     = csubst th p
    ...
    in case (lemma_evals_app_value (App (Prim Conj) thp) thq (Bc True) ev_thpq_tt ) of
      (AppRed _ id ev_and_id _ v' ev_thq_v') -> case (lemma_evals_app_value (Prim Conj) thp id ev_and_id) of
        (AppRed _ _ _ _thp v ev_thp_v) -> case (v, v') of 
          (Bc True, Bc True) -> undefined -- CHECKED (ev_thp_v, ev_thq_v')
          (Bc b,    Bc b')   -> undefined {- CHECKED impossible ("by lemma" ? lem_evals_val_det (csubst th pandq)
