...
{-@ lem_erase_th_sub :: g:Env -> t1:Type -> t2:Type -> ProofOf(Subtype g t1 t2) -> th:CSubst 
...
{-@ lem_unroll_ctsubst_tv_left :: th:CSubst -> { a:Vname | not (in_csubst a th) } 
        -> { t_a:Type | Set_emp (free t_a) && Set_emp (freeTV t_a) } ->  t:Type
        -> { pf:_ | ctsubst (CConsT a t_a th) t == tsubFTV a t_a (ctsubst th t) } @-}
lem_unroll_ctsubst_tv_left :: CSubst -> Vname -> Type -> Type -> Proof
...
{-@ lem_ctsubst_refn :: th:CSubst -> b:Basic -> x:Vname -> p:Expr
               -> { pf:_ | ctsubst th (TRefn b x p) == TRefn b x (csubst th p) } @-}
lem_ctsubst_refn :: CSubst -> Basic -> Vname -> Expr -> Proof
...
{-@ lem_ctsubst_poly :: th:CSubst -> a:Vname -> k:Kind -> t:Type
        -> { pf:_ | ctsubst th (TPoly a k t) == TPoly a k (ctsubst th t) } @-}  
lem_ctsubst_poly :: CSubst -> Vname -> Kind -> Type -> Proof
...
{-@ lem_ctsubst_self_refn :: th:CSubst -> b:Basic -> z:Vname -> p:Pred -> x:Vname 
        -> { pf:_ | ctsubst th (self (TRefn b z p) x) == 
		        TRefn b z (App (App (Prim And) (csubst th p)) 
                                       (App (App (equals b) (BV z)) (csubst th (FV x)))) } @-}
lem_ctsubst_self_refn :: CSubst -> Basic -> Vname -> Pred -> Vname -> Proof
...
{-@ lem_ctsubst_self_refn_notin :: th:CSubst -> b:Basic -> z:Vname -> p:Pred -> { x:Vname | not (in_csubst x th) }
        -> { pf:_ | ctsubst th (self (TRefn b z p) x) == self (ctsubst th (TRefn b z p)) x } @-}
lem_ctsubst_self_refn_notin :: CSubst -> Basic -> Vname -> Pred -> Vname -> Proof
...
{-@ lem_csubst_subBV :: x:Vname -> v:Value -> bt:FType 
           -> ProofOf(HasFType FEmpty v bt) -> th:CSubst -> p:Expr
           -> { pf:_ | csubst th (subBV x v p) == subBV x v (csubst th p) } @-}
lem_csubst_subBV :: Vname -> Expr -> FType -> HasFType -> CSubst -> Expr -> Proof
...
{-@ lem_ctsubst_tsubBV :: x:Vname -> v:Value -> bt:FType
           -> ProofOf(HasFType FEmpty v bt) -> th:CSubst -> t:Type
           -> { pf:_ | ctsubst th (tsubBV x v t) == tsubBV x v (ctsubst th t) } @-}
lem_ctsubst_tsubBV :: Vname -> Expr -> FType -> HasFType -> CSubst -> Type -> Proof
...
lem_csubst_and_unbind :: Vname -> Vname -> Expr -> FType -> HasFType -> CSubst -> Expr -> Proof
