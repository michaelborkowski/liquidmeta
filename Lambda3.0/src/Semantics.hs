...
altsIds (ACons dc e as)  = S.union (S.singleton (id dc)) (altsIds as)

{-@ reflect altsLookup @-}
{-@ altsLookup :: i:Id -> { as:Alts | Set_mem i (altsIds as) } -> Expr @-}
altsLookup :: Id -> Alts -> Expr
altsLookup i (ACons dc e as) | i == id dc  = e
                             | otherwise   = altsLookup i as

{-                     :: DataValue -> Alts -> Bool -}
{-@ predicate HasMatch DV AS =   Set_mem (id (dvdc DV)) (altsIds AS) && 
                                 argCount DV = arity (dvdc DV) @-}

{-@ reflect getMatch @-}
{-@ getMatch :: dv:DataValue -> { as:Alts | HasMatch dv as } -> Expr @-}
getMatch :: Expr -> Alts -> Expr
getMatch dv as = altsLookup (id (dvdc dv)) as
