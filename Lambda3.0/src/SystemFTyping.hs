...
{-@ reflect inst_dctypeF @-}
{-@ inst_dctypeF :: { t:Type | isTPoly t } -> { s:FType | isFTData s } -> FType @-}
inst_dctypeF :: Type -> FType -> FType
inst_dctypeF (TPoly k1 u) (FTData tc t1) = ftsubBV t1 (erase u)

{-@ reflect unapplyF @-}
{-@ unapplyF :: FEnv -> Vname -> xs:Names -> { s':FType | arrowsF s' == len xs } -> FEnv @-}
unapplyF :: FEnv -> Vname -> Names -> FType -> FEnv
unapplyF g y []     s'             =             FCons y s'  g
unapplyF g y (x:xs) (FTFunc u_x u) = unapplyF y (FCons x u_x g) xs (ftsubBV x u)

data AHasFType where        -- G, D, dds |- alts : t
    AFTEmpty :: FEnv -> Defs ->                   FType -> AHasFType
    AFTCons  :: FEnv -> Defs -> DDEfns -> Alts -> FType -> AHasFType
                      
-> Expr -> DCons -> Expr -> Type
-> AHasFType

{-@ data AHasFType
        AFTEmpty :: g:FEnv -> ds:Defs -> t:FType -> e:Expr
                      -> ProofOf(AHasFType g ds e DDEmpty AEmpty t)
        AFTCons  :: g:FEnv -> ds:Defs -> dds:DDefns -> as:Alts -> t:FType
                      -> ProofOf(AHasFType g ds dds as t)
                      -> e:Expr -> { s:FType | isFTData s } -> ProofOf(HasFType g ds e s)
                      -> dc:DCons -> e':Expr -> { ddt:Type | quant_arrows ddt == arity dc } .....

                      -> nms:Names 
                      -> ({ xs:Names | len xs == arity dc && Disjoint xs nms && Not }
                             -> { y:Vname | NotElem y (union xs nms) }
                             -> ProofOf(HasFType (unapplyF g y xs (inst_dctypeF ddt s)) ds e' t) )
                      -> ProofOf(AHasFType g ds (DDCons (DDef dc ddt) dds) (ACons dc e' as) t)
 @-}  


data HasFType where
    FTC    :: FEnv -> Defs -> Const -> HasFType
    FTVar1 :: FEnv -> Defs -> Vname -> FType -> HasFType 
    FTVar2 :: FEnv -> Defs -> Vname -> FType -> HasFType -> Vname -> FType -> HasFType
    FTVar3 :: FEnv -> Defs -> Vname -> FType -> HasFType -> Vname -> Kind -> HasFType
    FTAbs  :: FEnv -> Defs -> FType -> Kind -> WFFT -> Expr -> FType -> Names
                   -> (Vname -> HasFType) -> HasFType
    FTApp  :: FEnv -> Defs -> Expr -> FType -> FType -> HasFType 
                   -> Expr -> HasFType -> HasFType
    FTAbsT :: FEnv -> Defs -> Kind -> Expr -> FType -> Names -> (Vname -> HasFType) -> HasFType
    FTAppT :: FEnv -> Defs -> Expr -> Kind -> FType -> HasFType -> Type -> WFFT -> HasFType
    FTLet  :: FEnv -> Defs -> Expr -> FType -> HasFType -> Expr -> FType -> Names
                   -> (Vname -> HasFType) -> HasFType
    FTAnn  :: FEnv -> Defs -> Expr -> FType -> Type -> HasFType -> HasFType
    FTSwit :: FEnv -> Defs -> Expr -> Alts -> FType -> HasFType ??????

{-@ data HasFType where
        FTC    :: g:FEnv -> ds:Defs -> c:Const -> ProofOf(HasFType g ds (C c) (erase_const_ty c))
        FTVar1 :: g:FEnv -> ds:Defs -> { x:Vname | not (in_envF x g) } -> b:FType 
                    -> ProofOf(HasFType (FCons x b g) ds (FV x) b)
        FTVar2 :: g:FEnv -> ds:Defs -> { x:Vname | in_envF x g } 
                    -> b:FType -> ProofOf(HasFType g ds (FV x) b)
                    -> { y:Vname | y != x && not (in_envF y g) } -> b':FType 
                    -> ProofOf(HasFType (FCons y b' g) ds (FV x) b)
        FTVar3 :: g:FEnv -> ds:Defs -> { x:Vname | in_envF x g } 
                    -> b:FType -> ProofOf(HasFType g ds (FV x) b)
                    -> { y:Vname | y != x && not (in_envF y g) }  -> k:Kind
                    -> ProofOf(HasFType (FConsT y k g) ds (FV x) b)
        FTAbs  :: g:FEnv -> ds:Defs -> b:FType -> k:Kind -> ProofOf(WFFT g ds b k) 
                    -> e:Expr -> b':FType -> nms:Names
                    -> ( { y:Vname | NotElem y nms } -> ProofOf(HasFType (FCons y b g) ds (unbind y e) b') )
                    -> ProofOf(HasFType g ds (Lambda e) (FTFunc b b'))
        FTApp  :: g:FEnv -> ds:Defs -> e:Expr -> b:FType -> b':FType
                  -> ProofOf(HasFType g ds e (FTFunc b b')) 
                  -> e':Expr -> ProofOf(HasFType g ds e' b) 
                  -> ProofOf(HasFType g ds (App e e') b')
        FTAbsT :: g:FEnv -> ds:Defs -> k:Kind -> e:Expr -> b:FType -> nms:Names
                  -> ( { a':Vname | NotElem a' nms }
                         -> ProofOf(HasFType (FConsT a' k g) ds (unbind_tv a' e) (unbindFT a' b)) )
                  -> ProofOf(HasFType g ds (LambdaT k e) (FTPoly k b))
        FTAppT :: g:FEnv -> ds:Defs -> e:Expr -> k:Kind -> t':FType
                  -> ProofOf(HasFType g ds e (FTPoly k t')) 
                  -> { rt:UserType | Set_sub (free rt) (vbindsF g) && 
                                     Set_sub (freeTV rt) (tvbindsF g) && isLCT rt }
                  -> ProofOf(WFFT g ds (erase rt) k)
                  -> ProofOf(HasFType g ds (AppT e rt) (ftsubBV (erase rt) t'))
        FTLet  :: g:FEnv -> ds:Defs -> e_x:Expr -> b:FType -> ProofOf(HasFType g ds e_x b)
                  -> e:Expr -> b':FType -> nms:Names
                  -> ( { y:Vname | NotElem y nms } -> ProofOf(HasFType (FCons y b g) ds (unbind y e) b') )
                  -> ProofOf(HasFType g ds (Let e_x e) b')
        FTAnn  :: g:FEnv -> ds:Defs -> e:Expr -> b:FType 
                  -> { t1:Type | (erase t1 == b) && Set_sub (free t1) (vbindsF g) 
                                                 && Set_sub (freeTV t1) (tvbindsF g) && isLCT t1 }
                  -> ProofOf(HasFType g ds e b) -> ProofOf(HasFType g ds (Annot e t1) b) 
        FTSwit :: g:FEnv -> ds:Defs -> e:Expr -> { as:Alts | not (as == AEmpty) && ...... }

        ProofOf(HasFType g ds (Switch e as) ....... )
@-}

--        FTBC   :: g:FEnv -> ds:Defs -> b:Bool -> ProofOf(HasFType g ds (Bc b) (FTBasic TBool))
--        FTIC   :: g:FEnv -> ds:Defs -> n:Int -> ProofOf(HasFType g ds (Ic n) (FTBasic TInt))
--        FTPrm  :: g:FEnv -> ds:Defs -> c:Prim  -> ProofOf(HasFType g ds (Prim c) (erase_ty c))
