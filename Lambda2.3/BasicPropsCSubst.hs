...
-}
  ...
  ---     In the following, x can be either a term or type variable
{-
{-@ lem_remove_csubst :: th:CSub -> { x:Vname | in_csubst x th}
        -> { e:Expr | not (Set_mem x (fv e)) && not (Set_mem x (ftv e)) }
...
lem_remove_csubst (CCons z v_z th) x e = case ( x == z ) of
    ...
    (False) -> () ? lem_remove_csubst th x (subFV z v_z e)
lem_remove_csubst (CConsT a t_a th) x e = case ( x == a ) of
    (True)  -> () ? lem_subFTV_notin x t_a e
    (False) -> () ? lem_remove_csubst th x (subFTV a t_a e)
-}
{-@ lem_remove_cpsubst :: th:CSub -> { x:Vname | in_csubst x th}
        -> { ps:Preds | not (Set_mem x (fvP ps)) && not (Set_mem x (ftvP ps)) }
        -> { pf:Proof | cpsubst th ps == cpsubst (remove_fromCS th x) ps } @-}
lem_remove_cpsubst :: CSub -> Vname -> Preds -> Proof
lem_remove_cpsubst (CCons z v_z th) x ps = case ( x == z ) of
    (True)  -> () ? lem_psubFV_notin x v_z ps
    (False) -> () ? lem_remove_cpsubst th x (psubFV z v_z ps)
lem_remove_cpsubst (CConsT a t_a th) x ps = case ( x == a ) of
    (True)  -> () ? lem_psubFTV_notin x t_a ps
    (False) -> () ? lem_remove_cpsubst th x (psubFTV a t_a ps)

{-@ lem_remove_ctsubst :: th:CSub -> { x:Vname | in_csubst x th}
        -> { t:Type | not (Set_mem x (free t)) && not (Set_mem x (freeTV t)) }
...
lem_remove_ctsubst (CCons z v_z th) x t = case ( x == z ) of
    ...
    (False) -> () ? lem_remove_ctsubst th x (tsubFV z v_z t)
lem_remove_ctsubst (CConsT a t_a th) x t = case ( x == a ) of
    (True)  -> () ? lem_tsubFTV_notin x t_a t
