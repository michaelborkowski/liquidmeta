...
singleSub (TSub _ _ _ p_e_s _ _ _ _) = case p_e_s of
  (TSub {}) -> False
  _         -> True
...
tSubDepth (TSub _ _ _ p_e_s _ _ _ _) = tSubDepth p_e_s + 1
tSubDepth p_e_s                        = 1
...
lem_collapse_sub g e t p_g_wf p_e_t@(TSub _ _ s p_e_s _ k_t p_g_t p_s_t) = case p_e_s of
  (TSub _ _ w p_e_w _ k_s p_g_s p_w_s) -> lem_collapse_sub g e t p_g_wf p'_e_t      
    where
      p'_e_t = TSub g e w p_e_w t k_t p_g_t p_w_t
...
lem_invert_tabs g e t_x t' p_le_txt' p_g_wf = case p_le_txt' of
  (TAbs {}) -> p_le_txt'
  (TSub {}) 
    -> let (TSub _ _ s p_le_s _ k_t p_g_txt' p_s_t)
               = lem_collapse_sub g (Lambda e) (TFunc t_x t') p_g_wf p_le_txt'
         in case p_s_t of
              (SFunc _ _ _ p_tx_sx _ _  nms mk_p_s'_t') -> case p_le_s of
                (TAbs  _ s_x k_sx p_g_sx _ s' nms'  mk_p_e_s') 
                  ...
                  where
                    ...
                    mk_p_e_t' y = TSub (Cons y t_x g) (unbind y e) (unbindT y s') 
...
lem_invert_tabst g k e t' p_le_kt' p_g_wf = case p_le_kt' of
  (TAbsT {}) -> p_le_kt'
  (TSub {}) 
    -> let (TSub _ _ s p_le_s _ k_t p_g_kt' p_s_t)
               = lem_collapse_sub g (LambdaT k e) (TPoly k t') p_g_wf p_le_kt'
         in case p_s_t of
              (SPoly _ _ _ _ nms mk_p_s'_t')
                  -> TAbsT g k e t' nms''' mk_p_e_t'
                where
                  {-@ mk_p_e_t' :: { a:Vname | NotElem a nms''' }
                        -> { pf:HasType | propOf pf ==
                                HasType (ConsT a k g) (unbind_tv a e) (unbind_tvT a t') } @-}
                   mk_p_e_t' a = TSub (ConsT a k g) (unbind_tv a e) (unbind_tvT a s')
                                      (mk_p_e_s' a) (unbind_tvT a t') k_t' (mk_p_ag_t' a)
                                      (mk_p_s'_t' a)
                   (TAbsT _ _ _ s' nms'  mk_p_e_s')  = p_le_s
...
lem_lambdaT_tpoly_same_kind g k e k' t' p_ke_k't' p_g_wf = case p_ke_k't' of
  ...
  (TSub {})  -> case lem_collapse_sub g (LambdaT k e) (TPoly k' t') p_g_wf p_ke_k't' of
    (TSub _ _ s p_ke_s _ _ _ p_s_t) -> case p_s_t of
